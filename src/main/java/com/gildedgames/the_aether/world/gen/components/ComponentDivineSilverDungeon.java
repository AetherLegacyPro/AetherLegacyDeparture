...
public class ComponentDivineSilverDungeon extends AetherStructure {
	...
	public boolean generate() {
		...
		for (int tries = 0; tries < 100; tries++) {
			AetherGenUtils.generateClouds(this, BlocksAether.aercloud, 0, false, 10, this.random.nextInt(85), 0, this.random.nextInt(50), this.xTendency, this.zTendency);
		}
			...
			for (int q = 0; q < 3; q++) {
				for (int r = 0; r < 3; r++) {
					if (p == 0 && q != 0 && this.secondStaircaseZ == r) {
						...
						if (type >= 3) {
							this.setBlockWithOffset(7 + p * 7, -1 + q * 5, 7 + r * 7, BlocksAether.divine_angelic_trap, 0);
		...
		switch (item) {
			...
			case 10: {
				if (random.nextInt(4) == 0)
					return new ItemStack(ItemsAether.wynberry, random.nextInt(6) + 1, 2);
			}
			case 11: {
				if (random.nextInt(10) == 0)
