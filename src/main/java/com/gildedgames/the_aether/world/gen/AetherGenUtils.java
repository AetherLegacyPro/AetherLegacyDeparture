...
public class AetherGenUtils {
	...
	public static void generateGoldenFlower(AetherStructure structure, Block state, int meta, int offsetX, int offsetY, int offsetZ) {
		...
		for (int i = 0; i < 64; ++i) {
			...
			if (structure.getBlockStateWithOffset(x, y, z).getMaterial() == Material.air && y < 255 && structure.getBlockStateWithOffset(x, y - 1, z) == BlocksAether.enchanted_aether_grass) {
				structure.setBlockWithOffset(x, y, z, state, meta);
			}
		}
	}
	...
	public static void generateGoldenOakTree(AetherStructure structure, int offsetX, int offsetY, int offsetZ) {
		...
		if (structure.getBlockStateWithOffset(x, y - 1, z) != BlocksAether.enchanted_aether_grass && structure.getBlockStateWithOffset(x, y - 1, z) != BlocksAether.aether_grass) {
			return;
		}
