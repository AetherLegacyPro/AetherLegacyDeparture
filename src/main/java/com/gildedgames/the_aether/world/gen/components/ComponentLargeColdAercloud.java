...
public class ComponentLargeColdAercloud extends AetherStructure {
                    ...
                    for (int z1 = z; z1 < z + zMax; ++z1) {
                        if (this.getBlockStateWithOffset(x1, y1, z1) == Blocks.air) {
                            if (Math.abs(x1 - x) + Math.abs(y1 - y) + Math.abs(z1 - z) < 12 + shapeOffset) {
                                Block cloudBlock = this.getCloudBlockForPosition(x1, z1);
                                this.setBlockWithOffset(x1, y1, z1, cloudBlock, 0);
                            }
                        }
                    }
                }
    ...
    private Block getCloudBlockForPosition(int localX, int localZ) {
        if (this.worldObj == null) {
            return BlocksAether.aercloud;
        }

        int worldX = this.getXWithOffset(localX, localZ);
        int worldZ = this.getZWithOffset(localX, localZ);

        BiomeGenBase biome = this.worldObj.getBiomeGenForCoords(worldX, worldZ);
        if (biome != null && AetherWorld.stormy_skies != null && biome.biomeID == AetherWorld.stormy_skies.biomeID) {
