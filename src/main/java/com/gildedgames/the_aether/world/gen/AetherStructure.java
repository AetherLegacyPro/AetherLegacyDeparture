...
public abstract class AetherStructure extends StructureComponent {
    ...
    public boolean isBoxSolid(int x, int y, int z, int xRange, int yRange, int zRange) {
        ...
        for (int lineX = x; lineX < x + xRange; lineX++) {
            for (int lineY = y; lineY < y + yRange; lineY++) {
                for (int lineZ = z; lineZ < z + zRange; lineZ++) {
                    if (this.getBlockState(lineX + this.startX, lineY + this.startY, lineZ + this.startZ)
                        == Blocks.air) {
                        flag = false;
                    }
                }
    ...
    public TileEntity getTileEntityFromPosWithOffset(int x, int y, int z) {
        return !this.dungeonBoundingBox.isVecInside(this.getActualX(x, z), this.getActualY(y), this.getActualZ(x, z))
            ? null
            : this.worldObj.getTileEntity(this.getActualX(x, z), this.getActualY(y), this.getActualZ(x, z));
    }

    public Block getBlockStateWithOffset(int x, int y, int z) {
        return this.getBlockAtCurrentPosition(
            this.worldObj,
            x + this.startX,
            y + this.startY,
            z + this.startZ,
            this.dungeonBoundingBox);
    }
