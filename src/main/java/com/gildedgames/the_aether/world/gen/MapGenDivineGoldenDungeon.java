...
public class MapGenDivineGoldenDungeon extends MapGenStructure {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        if (randomTracker.testRandom(this.rand, 400) != 0) // fix spawn rates in next snapshot?
        {
            if (randomTracker.testRandom(this.rand, 430) != 0) {
                return false;
            }
        }

        return chunkX % 5 == 0 && chunkZ % 5 == 0;
    }
    ...
    public static class Start extends StructureStart {
        ...
        private void create(World worldIn, Random random, int chunkX, int chunkZ) {
            ...
            ComponentDivineGoldenDungeonIsland dungeon = new ComponentDivineGoldenDungeonIsland(
                (chunkX << 4) + 2,
                (chunkZ << 4) + 2);
            ...
            for (int stubIslands = 0; stubIslands < this.stubIslandCount; ++stubIslands) {
                ...
                this.components
                    .add(new ComponentDivineGoldenIslandStub((chunkX << 4) + 2, (chunkZ << 4) + 2, l4, k5, i6, 8));
            }

            this.components
                .add(new ComponentDivineGoldenDungeon((chunkX << 4) + 2, (chunkZ << 4) + 2, this.dungeonDirection));
