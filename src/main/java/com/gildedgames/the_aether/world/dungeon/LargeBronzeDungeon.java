...
public class LargeBronzeDungeon extends AetherDungeon {
    ...
    public boolean generateBossRoom(World world, Random random, int i, int j, int k) {
        if (!isBoxSolid(world, new PositionData(i, j - 3, k), new PositionData(16, 18, 16))
            || !isBoxSolid(world, new PositionData(i + 20, j, k + 2), new PositionData(12, 12, 12))) {
            return false;
        }
        ...
        switch (type) {
                ...
                if (world.getBlock(p, y + 2, q) == Blocks.air) {
                    ...
                    for (p = 0; p < 3 + random.nextInt(5); p++) {
                        chest.setInventorySlotContents(
                            ...
                            this.getNormalLoot(random));
                    }
                    for (p = 0; p < 3 + random.nextInt(5); p++) {
                        chestt.setInventorySlotContents(
                            random.nextInt(chestt.getSizeInventory()),
                            this.getNormalLoot(random));
                    }
                    // - -
                    ...
                    TileEntityMobSpawner tileentitymobspawner = (TileEntityMobSpawner) world
                        .getTileEntity(x + 7, y + 5, z + 7);

                    if (tileentitymobspawner != null) {
                        tileentitymobspawner.func_145881_a()
                            .setEntityName("aether_legacy.cyro");
                    }
                }
                break;
            }
            case 2: {
                if (world.getBlock(p, y + 2, q) == Blocks.air) {
                    ...
                    world.setBlock(x + 7, y + 5, z + 7, Blocks.mob_spawner);
                    TileEntityMobSpawner tileentitymobspawnerr = (TileEntityMobSpawner) world
                        .getTileEntity(x + 7, y + 5, z + 7);

                    if (tileentitymobspawnerr != null) {
                        tileentitymobspawnerr.func_145881_a()
                            .setEntityName("aether_legacy.cyro");
                    }
                }
                break;
        ...
        switch (switchCase) {
            case 1: {
                ...
                while (tunnelling) {
                    ...
                    if (hasBlock(
                        world,
                        new PositionData(x + 1, y, z),
                        new PositionData(2, 8, 6),
                        BlocksAether.ancient_carved_stone)
                        || hasBlock(
                            world,
                            new PositionData(x + 1, y, z),
                            new PositionData(2, 8, 6),
                            BlocksAether.locked_ancient_carved_stone)) {
                        tunnelling = false;
                    }
            ...
            case 3: {
                ...
                while (tunnelling) {
                    ...
                    if (hasBlock(
                        world,
                        new PositionData(x, y, z + 1),
                        new PositionData(6, 8, 2),
                        BlocksAether.ancient_carved_stone)
                        || hasBlock(
                            world,
                            new PositionData(x, y, z + 1),
