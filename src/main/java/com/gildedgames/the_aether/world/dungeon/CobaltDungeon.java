...
public class CobaltDungeon extends AetherDungeon {
    ...
    public boolean generateBossRoom(World world, Random random, int i, int j, int k) {
        if (!isBoxSolid(world, new PositionData(i, j - 3, k), new PositionData(16, 16, 16))
            || !isBoxSolid(world, new PositionData(i + 20, j, k + 2), new PositionData(12, 10, 12))) {
            return false;
        }
            ...
            case 1: {
                ...
                while (tunnelling) {
                    ...
                    if (hasBlock(
                        ...
                        new PositionData(2, 6, 6),
                        BlocksAether.caelestia_stone)
                        || hasBlock(
                            world,
                            new PositionData(x + 1, y, z),
                            new PositionData(2, 6, 6),
                            BlocksAether.oblitus_stone)) {
                        tunnelling = false;
                    }
    ...
    private ItemStack getNormalLoot(Random random) {
        ...
        return new ItemStack(BlocksAether.oblitus_stone, random.nextInt(2) + 3);
    }
