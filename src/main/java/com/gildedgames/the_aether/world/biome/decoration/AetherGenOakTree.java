...
public class AetherGenOakTree extends WorldGenAbstractTree {
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        if (world.getBlock(x, y - 1, z) != BlocksAether.aether_grass
            && world.getBlock(x, y - 1, z) != BlocksAether.enchanted_aether_grass
            && world.getBlock(x, y - 1, z) != BlocksAether.aether_dirt) {
            return false;
        }
        ...
        for (int x1 = x - 3; x1 < x + 4; x1++) {
            for (int y1 = y + 5; y1 < y + 12; y1++) {
                for (int z1 = z - 3; z1 < z + 4; z1++) {
                    if ((x1 - x) * (x1 - x) + (y1 - y - 8) * (y1 - y - 8) + (z1 - z) * (z1 - z) < 12 + random.nextInt(5)
