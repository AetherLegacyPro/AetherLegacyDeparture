...
public class GoldenIsland extends WorldGenerator {
    ...
    public GoldenIsland(int bumpsWide1, int bumpsLong1, int bumpHeightVarTop1, int bumpHeightVarBottom1,
    ...
    private Block materialAtLevel(Random rand, int i, boolean flag) {
        ...
        if (i < j) {
            return rand.nextInt(20) == 0 ? BlocksAether.enchanted_holystone : BlocksAether.holystone;
        }
        if (i == j) {
    ...
    private void generateOre(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = 0; i < 8 + rand.nextInt(radius); ++i) {
            ...
            if (world.getBlock(x1, y1, z1)
                .isReplaceableOreGen(world, x, y, z, BlocksAether.holystone)) {
                ...
                for (int yTest = y1; yTest < 128; ++yTest) {
                    if (world.getBlock(x1, yTest, z1)
                        ...
                        && (world.getBlock(x1, yTest - 1, z1) == BlocksAether.enchanted_aether_grass
                            || world.getBlock(x1, yTest - 1, z1) == BlocksAether.aether_dirt)) {
    ...
    private static AetherGenGoldenFruitTree tree = new AetherGenGoldenFruitTree(
        BlocksAether.golden_oak_leaves,
        BlocksAether.golden_oak_fruit_leaves,
        50,
        5,
        true);
    ...
    private void generateTallGrass(World world, Random rand, int x, int y, int z) {
        for (int i1 = 0; i1 < 16; ++i1) {
            ...
            if (l1 > 100) {
                ...
                switch (chance) {
                    case 1:
                        (new WorldGenTallGrass(BlocksAether.enchanted_aether_tallgrass, 1))
                            .generate(world, rand, j1, l1, k1);
                        break;
                    case 2:
                        (new WorldGenTallGrass(BlocksAether.enchanted_aether_tulips, 1))
                            .generate(world, rand, j1, l1, k1);
                        break;
                    case 3:
                        (new WorldGenTallGrass(BlocksAether.enchanted_bloom, 1)).generate(world, rand, j1, l1, k1);
                        break;
                    case 4:
                        (new WorldGenTallGrass(BlocksAether.enchanted_quickshoot, 1)).generate(world, rand, j1, l1, k1);
                        break;
                    case 5:
                        (new WorldGenTallGrass(BlocksAether.enchanted_aether_tallgrass, 1))
                            .generate(world, rand, j1, l1, k1);
