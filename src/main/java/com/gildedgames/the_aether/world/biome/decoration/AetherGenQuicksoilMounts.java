...
public class AetherGenQuicksoilMounts extends WorldGenAbstractTree {
    ...
    public AetherGenQuicksoilMounts(final Block leafID, final Block logID, final int logMeta) {
        super(true);
    ...
    public AetherGenQuicksoilMounts() {
        super(false);
    ...
    public boolean generate(final World world, final Random random, final int i, final int j, final int k) {
        ...
        if (this.moundOnly || this.isQuicksoilDunesBiome(biome)) {
            return this.generateQuicksoilMound(world, random, i, j, k);
        }
    ...
    private boolean generateQuicksoilMound(final World world, final Random random, final int x, final int y, final int z) {
        ...
        int moundHeight = random.nextInt(12) + 8;
        ...
        for (int dx = -radiusX; dx <= radiusX; dx++) {
            for (int dz = -radiusZ; dz <= radiusZ; dz++) {
                ...
                for (int dy = 0; dy < columnHeight; dy++) {
                    ...
                    Block moundBlock = BlocksAether.quicksoil;
    ...
    private boolean canReplaceForMound(Block block) {
        return block == Blocks.air || block == BlocksAether.holystone;
    }
}
