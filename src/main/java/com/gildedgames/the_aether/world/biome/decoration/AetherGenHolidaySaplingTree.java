...
public class AetherGenHolidaySaplingTree extends WorldGenAbstractTree {

	public AetherGenHolidaySaplingTree(boolean notify) {
		super(notify);
	}
	...
	public boolean generate(World world, Random random, int x, int y, int z) {
			...
			for (int k3 = x - i3; k3 <= x + i3; k3++) {
				...
				for (int i4 = z - i3; i4 <= z + i3; i4++) {
					...
					if ((Math.abs(l3) != i3 || Math.abs(j4) != i3 || random.nextInt(2) != 0 && j2 != 0) && !world.getBlock(k3, k1, i4).isOpaqueCube()) {
						this.setBlockAndNotifyAdequately(world, k3, k1, i4, BlocksAether.holiday_leaves, 0);
					}
				}
		...
		for (int l1 = 0; l1 < l; l1++) {
			...
			if (k2 == Blocks.air || k2 == BlocksAether.holiday_leaves) {
