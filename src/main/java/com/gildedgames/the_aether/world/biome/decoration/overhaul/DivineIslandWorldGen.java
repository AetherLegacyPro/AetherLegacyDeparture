...
public class DivineIslandWorldGen implements IWorldGenerator {
    ...
    int aether_random = AetherConfig.divine_island_rarity;

    public void generateAether(World world, Random rand, int x, int z) {
        if (Math.abs(x) < 1500 && Math.abs(z) < 1500) return;

        BiomeGenBase biome = world.getWorldChunkManager()
            .getBiomeGenAt(x + 16, z + 16);
        if ((biome == AetherWorld.aether_biome) && AetherConfig.divine_island_rarity != 0
            && rand.nextInt(aether_random) == 0
            && AetherConfig.divine_island_enable == true) {
            byte byte0 = 20;
            byte byte1 = 20;
            byte byte2 = 35;
            byte byte3 = 35;
            ...
            if (rand.nextInt(byte21) == 0) {
                ...
                Block genOre = BlocksAether.amethyst_glowstone;
                int chance = (int) (1 + Math.random() * 6);
                switch (chance) {
                    case 1:
                        (new DivineIsland(
                            bumpsWide1,
                            bumpsLong1,
                            bumpHeightVarTop1,
                            bumpHeightVarBottom1,
                            bumpHeightMinTop1,
                            bumpHeightMinBottom1,
                            blocksPerBumpX1,
                            blocksPerBumpZ1,
                            blocksPerBumpTopY1,
                            blocksPerBumpBottomY1,
                            blurPassesTop1,
                            blurPassesBottom1,
                            spikeHeightVarTop1,
                            spikeHeightVarBottom1,
                            radialDistanceSamples1,
                            radialDistanceMin1,
                            radialDistanceVar1,
                            radialDistanceScaling1,
                            radialDistanceBlurPasses1,
                            genOre)).generate(world, rand, x1, y1, z1);
                        (new DivineIsland(
                            bumpsWide1,
                            bumpsLong1,
                            bumpHeightVarTop1,
                            ...
                            radialDistanceScaling1,
                            radialDistanceBlurPasses1,
                            genOre)).generate(world, rand, x1 - 32, y1, z1 + 32);
                        (new DivineIsland(
                            bumpsWide1,
                            bumpsLong1,
                            bumpHeightVarTop1,
