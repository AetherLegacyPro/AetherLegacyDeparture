...
public class AetherGenCrystalTree extends WorldGenAbstractTree {
	...
	public boolean branch(World world, Random random, int x, int y, int z, int slant) {
		...
		for (int n = 0; n < random.nextInt(2); n++) {
			...
			if (world.getBlock(x, y, z) == BlocksAether.crystal_leaves) {
				world.setBlock(x, y, z, BlocksAether.skyroot_log);
				world.setBlock(i, y, k, BlocksAether.skyroot_log);
			}
		}
	...
	public boolean generate(World world, Random random, int x, int y, int z) {
		if (world.getBlock(x, y - 1, z) != BlocksAether.arctic_grass) {
			return false;
		}
			...
			for (int y1 = y + 5; y1 < y + 12; y1++) {
				for (int z1 = z - 3; z1 < z + 4; z1++) {
					if ((x1 - x) * (x1 - x) + (y1 - y - 8) * (y1 - y - 8) + (z1 - z) * (z1 - z) < 12 + random.nextInt(5) && world.isAirBlock(x1, y1, z1)) {
						world.setBlock(x1, y1, z1, BlocksAether.crystal_leaves);
					}
				}
			}
		}
		...
		for (int n = 0; n < height; n++) {
			...
			world.setBlock(x, y + n, z, BlocksAether.skyroot_log);
		}
