...
public class AetherBiomeAetherForest extends BiomeGenBase {

    @SuppressWarnings("unchecked")
    public AetherBiomeAetherForest() {
        super(AetherConfig.getAetherForestBiomeID());
        ...
        this.setBiomeName("Aether Forest");
        this.setDisableRain();
        this.setColor(0);
    }

    public WorldGenAbstractTree getRandomTreeFeature(final Random random) {
        final int ratio = random.nextInt(110);
        if (ratio <= 35) {
            return new AetherGenMassiveTree(BlocksAether.skyroot_leaves, 12, false);
        }  else if (ratio <= 45) {
            return new AetherGenMassiveTree(BlocksAether.skyroot_leaves, 8, false);
        }  else if (ratio <= 68) {
            return new AetherGenMassiveTree(BlocksAether.blue_skyroot_leaves, 8, false);
        } else if (ratio <= 69) {
            ...
            return new AetherGenFruitTree(BlocksAether.skyroot_leaves, BlocksAether.skyroot_leaves, 50, 5, true);
        }  else if (ratio <= 77) {
            return new AetherGenMassiveTree(BlocksAether.skyroot_leaves, 18, false);
        } else if (ratio <= 78) {
            return new AetherGenFruitTree(BlocksAether.blue_skyroot_leaves, BlocksAether.blue_skyroot_leaves, 25, 5, true);
        } else if (ratio <= 87) {
            return new AetherGenMassiveTree(BlocksAether.skyroot_leaves, 25, true);
        }

        return new AetherGenMassiveTree(BlocksAether.skyroot_leaves, 35, true);
    }
