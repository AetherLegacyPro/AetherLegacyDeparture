...
public class AetherGenOverhaulLate implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		...
		public void generateAetherLate(World world, Random rand, int x, int z) {
			...
			this.generateOre(BlocksAether.continuum_ore, world, rand, x, z, 2, 4, 7, 32, 128, BlocksAether.holystone);
            ...
            int x1 = x * 16 + rand.nextInt(8) + 8;
            int z1 = z * 16 + rand.nextInt(8) + 8;
            BiomeGenBase biome;
            biome = world.getBiomeGenForCoords(x1, z1);

            if (biome == AetherWorld.enchanted_island) {
                this.generateOre(BlocksAether.empyrean_ore, world, rand, x, z, 3, 6, 7, 32, 128, BlocksAether.holystone);
            }

            if (biome == AetherWorld.divine_island) {
                this.generateOre(BlocksAether.empyrean_ore, world, rand, x, z, 3, 6, 7, 32, 256, BlocksAether.holystone);
                this.generateOre(BlocksAether.empyrean_ore_2, world, rand, x, z, 3, 6, 7, 64, 256, BlocksAether.enchanted_holystone);
            }
			...
			if (AetherConfig.enableAetheralStone() && biome == AetherWorld.enchanted_island) {
                ...
                this.generateOre(BlocksAether.aetheral_continuum_ore, world, rand, x, z, 3, 4, 8, 32, 196, BlocksAether.aetheral_stone);
			...
			if (AetherConfig.enableAgisoite() && (biome == AetherWorld.aether_biome || biome == AetherWorld.aether_forest)) {
			    ...
			    this.generateOre(BlocksAether.agiosite_continuum_ore, world, rand, x, z, 3, 4, 8, 16, 66, BlocksAether.agiosite);
			...
			if (AetherConfig.enableDeific() && biome == AetherWorld.divine_island) {
			    this.generateOre(BlocksAether.deific_ambrosium_ore, world, rand, x, z, 12, 13, 11, 0, 48, BlocksAether.deific);
			    this.generateOre(BlocksAether.deific_zanite_ore, world, rand, x, z, 8, 9, 10, 0, 48, BlocksAether.deific);
			    this.generateOre(BlocksAether.deific_arkenium_ore, world, rand, x, z, 4, 5, 6, 0, 48, BlocksAether.deific);
			    this.generateOre(BlocksAether.deific_continuum_ore, world, rand, x, z, 3, 4, 8, 0, 48, BlocksAether.deific);
			    this.generateOre(BlocksAether.deific_gravitite_ore, world, rand, x, z, 3, 4, 5, 0, 34, BlocksAether.deific);
			}

			if(AetherConfig.enableAuraliteGeos()) {
				int x8;
				int z8;
				x8 = x * 16 + rand.nextInt(8) + 8;
				z8 = z * 16 + rand.nextInt(8) + 8;
				if(rand.nextInt(80) == 0 && biome == AetherWorld.enchanted_island) {
					new AuraliteGeoGen().generate(world, rand, x8, rand.nextInt(48) + 24, z8);
				}

                if(rand.nextInt(80) == 0 && biome == AetherWorld.divine_island) {
                    new AuraliteGeoGen().generate(world, rand, x8, rand.nextInt(64) + 64, z8);
                }
