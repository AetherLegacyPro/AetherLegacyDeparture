...
public class AetherGenGreenClouds extends WorldGenerator {

	private int GreenCloudAmount;

	private int GreenCloudMeta;

	public AetherGenGreenClouds() {

	}

	public void setGreenCloudAmount(int amount) {
		this.GreenCloudAmount = amount;
	}

	public void setGreenCloudMeta(int meta) {
		this.GreenCloudMeta = meta;
	}
	...
	public boolean generate(World worldIn, Random randIn, int xIn, int yIn, int zIn) {
		...
		for (int amount = 0; amount < this.GreenCloudAmount; ++amount) {
					...
					for (int z = zIn; z < zIn + randIn.nextInt(2) + 3; ++z) {
						if (worldIn.isAirBlock(x, y, z)) {
							if (Math.abs(x - xIn) + Math.abs(y - yIn) + Math.abs(z - zIn) < 4 + randIn.nextInt(10)) {
								this.setBlockAndNotifyAdequately(worldIn, x, y, z, BlocksAether.green_aercloud, this.GreenCloudMeta);
							}
