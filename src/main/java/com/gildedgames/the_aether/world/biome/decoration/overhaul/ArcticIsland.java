...
public class ArcticIsland extends WorldGenerator {
    ...
    public ArcticIsland(int bumpsWide1, int bumpsLong1, int bumpHeightVarTop1, int bumpHeightVarBottom1,
    ...
    private void generateOre(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = 0; i < 8 + rand.nextInt(radius); ++i) {
            ...
            if (world.getBlock(x1, y1, z1)
                ...
                world.setBlock(x1, y1, z1, BlocksAether.zanite_ore);

                for (int yTest = y1; yTest < 128; ++yTest) {
                    if (world.getBlock(x1, yTest, z1)
                        .isReplaceable(world, x1, yTest, z1)
                        && (world.getBlock(x1, yTest - 1, z1) == BlocksAether.arctic_grass
    ...
    private static AetherGenLargeWisprootTree tree = new AetherGenLargeWisprootTree(
        BlocksAether.blue_light_skyroot_leaves,
        BlocksAether.wisproot_log,
        0);
    private static AetherGenWisprootTree tree2 = new AetherGenWisprootTree(
        BlocksAether.blue_light_skyroot_leaves,
        8,
        false);
    ...
    private void generateTrees(World world, Random rand, int x, int y, int z) {
        ...
        int k1;
        int l1;
        for (i6 = 0; i6 < 12 + rand.nextInt(radius); ++i6) {
            ...
            if (l1 > 100) {
                ...
                switch (chance) {
                    case 1:
                        ...
                        break;
                    case 2:
                        (new WorldGenTallGrass(BlocksAether.white_rose, 1)).generate(world, rand, j1, l1, k1);
                        break;
                    case 3:
                        (new WorldGenTallGrass(BlocksAether.white_flower, 1)).generate(world, rand, j1, l1, k1);
                        break;
                    default:
                        (new WorldGenTallGrass(BlocksAether.arctic_tallgrass, 1)).generate(world, rand, j1, l1, k1);
                        break;
