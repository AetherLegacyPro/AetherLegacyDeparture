...
public class TileEntityAmplifier extends AetherTileEntity {
        ...
        if (this.currentAmplifier != null) {
            if (this.progress >= this.ticksRequired) {
                if (!this.worldObj.isRemote) {
                    ItemStack result = this.currentAmplifier.getOutput()
                        .copy();

                    EnchantmentHelper
                        .setEnchantments(EnchantmentHelper.getEnchantments(this.getStackInSlot(0)), result);

                    if (this.getStackInSlot(0)
                        .hasTagCompound()) {
                    ...
                    if (this.getStackInSlot(0)
                        .getItem()
                        .hasContainerItem(this.getStackInSlot(0))) {
                        this.setInventorySlotContents(
                            0,
                            this.getStackInSlot(0)
                                .getItem()
                                .getContainerItem(this.getStackInSlot(0)));
                    } else {
            ...
            if (this.getStackInSlot(0) == null || (this.getStackInSlot(0) != null && AetherAPI.instance()
                .getAmplifier(this.getStackInSlot(0)) != this.currentAmplifier)) {
                this.currentAmplifier = null;
                this.progress = 0;
            }

            if (this.powerRemaining <= 0) {
                if (this.getStackInSlot(1) != null && AetherAPI.instance()
                    .isAmplifierFuel(this.getStackInSlot(1))) {
                    this.powerRemaining += AetherAPI.instance()
                        .getAmplifierFuel(this.getStackInSlot(1))
                        .getTimeGiven();
            ...
            AetherAmplifier amplifier = AetherAPI.instance()
                .getAmplifier(itemstack);

            if (amplifier != null) {
                if (this.getStackInSlot(2) == null || (amplifier.getOutput()
                    .getItem()
                    ...
                    this.ticksRequired = AetherHooks
                        .onSetAmplifierTime(this, this.currentAmplifier, this.ticksRequired);
                }
            }
        }
    ...
    public void addEnchantmentWeight(ItemStack stack) {
        Map<Enchantment, Integer> enchantments = (Map<Enchantment, Integer>) (Map<?, ?>) EnchantmentHelper
            .getEnchantments(stack); // returns Integer, Integer
    ...
    public boolean isValidSlotItem(int slot, ItemStack stackInSlot) {
        if (slot == 2) {
            return false;
        } else if (slot == 1 && AetherAPI.instance()
            .isAmplifierFuel(stackInSlot)) {
                return true;
            } else if (slot == 0 && AetherAPI.instance()
                .hasAmplifier(stackInSlot)) {
                    return true;
                }
