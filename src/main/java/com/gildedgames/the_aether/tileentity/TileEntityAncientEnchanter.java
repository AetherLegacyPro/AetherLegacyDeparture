...
public class TileEntityAncientEnchanter extends TileEntity implements IInventory {
    ...
    public void addEnchantable(final EntityPlayer player, final ItemStack stack) {
        ...
        } else if (enchantableStack.getItem() == stack.getItem()
            && enchantableStack.getItemDamage() == stack.getItemDamage()) {
                    ...
                    if (this.worldObj.isRemote) {
                        FMLClientHandler.instance()
                            .getClient().ingameGUI.getChatGUI()
                                .printChatMessage(new ChatComponentText("Altar is at full capacity."));
                    }
                } else {
    ...
    public boolean canCombineStackWithRemainder(final ItemStack newStack, final ItemStack currentStack,
        final int stackSizeLimit) {
        return newStack.getItem() == currentStack.getItem() && newStack.getItemDamage() == currentStack.getItemDamage()
            && !this.stackIsFull(currentStack, stackSizeLimit)
            && currentStack.stackSize + newStack.stackSize > stackSizeLimit;
    }
    ...
    public void combineStackWithRemainder(final ItemStack newStack, final ItemStack currentStack,
        final int stackSizeLimit) {
        if (!this.canCombineStackWithRemainder(newStack, currentStack, stackSizeLimit)) {
            return;
        }
    ...
    public void updateEntity() {
        if (!this.worldObj.isRemote) {
            if (this.renderedItem == null) {
                if (this.getEnchanterStacks(0) != null) {
                    this.renderedItem = new EntityFakeItem(
                        this.worldObj,
                        this.xCoord + 0.5,
                        this.yCoord + 1.15,
                        this.zCoord + 0.5,
                        this.getEnchanterStacks(0));
        ...
        if (this.currentEnchantment != null) {
            ...
            if (this.canEnchant() && fuelStack != null
                && fuelStack.stackSize >= this.currentEnchantment.enchantAmbrosiumNeeded
                && inputStack != null) {
            ...
            for (final AetherEnchantmentAncientEnchanter enchantment : TileEntityAncientEnchanter.enchantments) {
                ...
                if (itemstack != null && itemstack.getItem() == enchantment.getRequirement()
                    .getItem()
                    && (itemstack.getItemDamage() == enchantment.getRequirement()
                        .getItemDamage() || enchantment.repairing)) {
                    if (this.enchanterItemStacks[2] == null) {
                        this.currentEnchantment = enchantment;
                    } else {
                        if (this.enchanterItemStacks[2].getItem() != result.getItem() || result.getItem()
                            .getItemStackLimit() <= this.enchanterItemStacks[2].stackSize) {
                            continue;
                        }
                        this.currentEnchantment = enchantment;
                    }
    ...
    public boolean isEnchantable(final ItemStack stack) {
        for (final AetherEnchantmentAncientEnchanter enchantment : TileEntityAncientEnchanter.enchantments) {
            if (stack != null && enchantment != null
                && stack.getItem() == enchantment.getRequirement()
                    .getItem()
                && (stack.getItemDamage() == enchantment.getRequirement()
                    .getItemDamage() || enchantment.repairing)) {
                return true;
            }
        }
    ...
    private void sendToAllInOurWorld(final Packet pkt) {
        final ServerConfigurationManager scm = FMLCommonHandler.instance()
