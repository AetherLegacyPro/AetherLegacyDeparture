...
public class EntityElysianGuardian extends EntityMob {
    ...
    protected void onDeathUpdate() {
        if (this.worldObj.isRemote) {
            for (int i = 0; i < 3; ++i) {
                NewAetherParticleHandler.ELYSIAN_TELEPORT.spawn(
                    worldObj,
                    this.posX + (this.rand.nextDouble() - 1.5D) * (double) this.width,
                    this.posY + this.rand.nextDouble() * (double) this.height + 0.25D,
                    this.posZ + (this.rand.nextDouble() - 1.5D) * (double) this.width);
                NewAetherParticleHandler.ELYSIAN_TELEPORT.spawn(
                    worldObj,
                    this.posX + (this.rand.nextDouble() - 1.5D) * (double) this.width,
                    this.posY + this.rand.nextDouble() * (double) this.height + 1.0D,
                    this.posZ + (this.rand.nextDouble() - 1.5D) * (double) this.width);
                NewAetherParticleHandler.ELYSIAN_TELEPORT.spawn(
                    worldObj,
                    this.posX + (this.rand.nextDouble() - 1.5D) * (double) this.width,
                    this.posY + this.rand.nextDouble() * (double) this.height + 2.0D,
                    this.posZ + (this.rand.nextDouble() - 1.5D) * (double) this.width);
                NewAetherParticleHandler.ELYSIAN_TELEPORT.spawn(
                    worldObj,
                    this.posX + (this.rand.nextDouble() - 1.5D) * (double) this.width,
                    this.posY + this.rand.nextDouble() * (double) this.height + 3.0D,
                    this.posZ + (this.rand.nextDouble() - 1.5D) * (double) this.width);
            }
        }
    }
    ...
    public void onUpdate() {
        ...
        if (!this.onGround && this.getAttackTarget() != null
            ...
            && this.motionY < 0.0D
            && getDistanceToEntity(this.getAttackTarget()) <= 30F
            && canEntityBeSeen(this.getAttackTarget())) {
    ...
    protected void attackEntity(final Entity entity1, final float f) {
        ...
        if (this.getHealth() <= 65.0f) {
            ...
            if (this.timeSinceIgnited == 0) {
                ...
                this.motionZ = 0.0;
                this.worldObj
                    .playSoundAtEntity(this, "random.fizz", 1.3f, 0.3f / (this.rand.nextFloat() * 0.4f + 0.8f));
            }
            ...
            if (this.timeSinceIgnited >= 75) {
                ...
                for (int j = 0; j < 9; ++j) {
                    ...
                    final EntityElysianGuardianLaser entityDivineMissile = xcEntityMobDivineMissile = new EntityElysianGuardianLaser(
                        this.worldObj,
                        this);
                    xcEntityMobDivineMissile.posY += 2.099999976158142;
                    final double d9 = entity1.posY + entity1.getEyeHeight()
                        - 0.40000000298023225
                        - entityDivineMissile.posY;
                    final float f5 = MathHelper.sqrt_double(d7 * d7 + d8 * d8) * 0.0f;
                    this.worldObj.playSoundAtEntity(
                        this,
                        "aether_legacy:projectile.laser.laser_fire",
                        0.9f,
                        0.9f / (this.rand.nextFloat() * 0.4f + 0.8f));
    ...
    public boolean attackEntityAsMob(Entity entity) {
        ...
        if (this.attackTime <= 0 && entity.boundingBox.maxY > this.boundingBox.minY
            ...
            this.worldObj.playSoundAtEntity(
                this,
                "aether_legacy:projectile.charged_hit",
                0.9f,
                0.9f / (this.rand.nextFloat() * 0.4f + 0.8f));
            swingArm();
