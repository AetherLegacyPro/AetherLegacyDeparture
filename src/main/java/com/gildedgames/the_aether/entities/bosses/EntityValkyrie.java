...
public class EntityValkyrie extends EntityAetherMob {
    ...
    public void onEntityUpdate() {
        ...
        if (this.teleTimer >= 450) {
            ...
            this.teleTimer = 446;
        } else if ((this.teleTimer % 5) == 0 && this.getAttackTarget() != null
            && !canEntityBeSeen(this.getAttackTarget())) {
                this.teleTimer += 100;
            }
    ...
    public void onUpdate() {
        ...
        if (!this.onGround && this.getAttackTarget() != null
            ...
            && this.motionY < 0.0D
            && getDistanceToEntity(this.getAttackTarget()) <= 16F
            && canEntityBeSeen(this.getAttackTarget())) {
        ...
        if (this.worldObj.difficultySetting == EnumDifficulty.PEACEFUL
            && (this.getAttackTarget() != null || this.angerLevel > 0)) {
            this.angerLevel = 0;
            this.setAttackTarget(null);
        }
            ...
            if (player.getHealth() <= 0 && player.isDead) {
                ...
                if (pokey == 2) {
                    ...
                    chatItUp(
                        player,
                        StatCollector
                            .translateToLocalFormatted("gui.valkyrie.dialog.playerdead.2", player.getDisplayName()));
                } else {
        ...
        if (this.attackTime <= 0 && entity.boundingBox.maxY > this.boundingBox.minY
            ...
            if (entity != null && this.getAttackTarget() != null
                ...
                if (player.getHealth() <= 0) {
                    ...
                    if (pokey == 2) {
                        chatItUp(
                            (EntityPlayer) this.getAttackTarget(),
                            StatCollector.translateToLocal("gui.valkyrie.dialog.playerdead.1"));
                    } else if (pokey == 1) {
                        chatItUp(
                            (EntityPlayer) this.getAttackTarget(),
                            StatCollector.translateToLocalFormatted(
                                "gui.valkyrie.dialog.playerdead.2",
                                player.getDisplayName()));
                    } else {
                        chatItUp(
                            (EntityPlayer) this.getAttackTarget(),
                            StatCollector.translateToLocal("gui.valkyrie.dialog.playerdead.3"));
                    }
    ...
    protected void dropFewItems(boolean var1, int var2) {
        ...
        dropItem(ItemsAether.valkyrie_nugget, 2);
    }
    ...
    public boolean getCanSpawnHere() {
        ...
        return (this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_light_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_divine_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_divine_light_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_mythic_light_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_mythic_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_ancient_angelic_stone
            || this.worldObj.getBlock(i, j - 1, k) == BlocksAether.locked_ancient_light_angelic_stone)
            && this.worldObj.getBlockLightValue(i, j, k) < 14
            && canSpawn;
