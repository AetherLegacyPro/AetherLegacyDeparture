...
import com.gildedgames.the_aether.client.gui.dialogue.entity.GuiAncientValkyrieDialogue;
import com.gildedgames.the_aether.entities.ai.EntityAIAttackContinuously;
import com.gildedgames.the_aether.entities.ai.valkyrie_queen.AncientValkyrieQueenAIWander;
...
public class EntityAncientValkyrieQueen extends EntityBossMob implements IAetherBoss {
    ...
    public EntityAncientValkyrieQueen(World world) {
        super(world);

        this.timeUntilTeleport = this.rand.nextInt(200);
    ...
    public EntityAncientValkyrieQueen(World world, double x, double y, double z) {
        this(world);
        this.safeX = posX = x;
        this.safeY = posY = y;
        this.safeZ = posZ = z;
    ...
    public void registerEntityAI() {
        ...
        this.tasks.addTask(2, new AncientValkyrieQueenAIWander(this, 0.5D));
    ...
    private void unlockTreasure() {
        this.worldObj.setBlock(this.dungeonX + 16, dungeonY + 1, dungeonZ + 9, BlocksAether.wisproot_trapdoor, 3, 2);
        this.worldObj.setBlock(this.dungeonX + 17, dungeonY + 1, dungeonZ + 9, BlocksAether.wisproot_trapdoor, 2, 2);
        this.worldObj.setBlock(this.dungeonX + 16, dungeonY + 1, dungeonZ + 10, BlocksAether.wisproot_trapdoor, 3, 2);
        this.worldObj.setBlock(this.dungeonX + 17, dungeonY + 1, dungeonZ + 10, BlocksAether.wisproot_trapdoor, 2, 2);
        ...
        for (int x = this.dungeonX - 27; x < this.dungeonX + 30; x++) {
            for (int y = this.dungeonY - 1; y < this.dungeonY + 22; y++) {
                for (int z = this.dungeonZ - 6; z < this.dungeonZ + 26; z++) {
                    ...
                    if (block == BlocksAether.locked_ancient_angelic_stone
    ...
    public void makeHomeShot(int shots, EntityPlayer player) {
        for (int i = 0; i < shots; i++) {
            EntityCrystal crystal = new EntityCrystal(
                this.worldObj,
                this.posX - (this.motionX / 2D),
                this.posY,
                this.posZ - (this.motionZ / 2D),
                player);
    ...
    public void displayAncientValkyrieDialogue() {
        if (this.worldObj.isRemote) {
            FMLClientHandler.instance()
                .getClient()
                .displayGuiScreen(new GuiAncientValkyrieDialogue(this));
        }
    }
    ...
    public boolean interact(EntityPlayer entityplayer) {
        ...
        if (this.isBossReady()) {
            ...
            this.displayAncientValkyrieDialogue();
        ...
        if (!this.isBossReady()) {
                ...
                if (this.getEntityToAttack() instanceof EntityPlayer target) {
                    ...
                    if (target != null) {
                        ...
                        if (this.timeUntilTeleport++ >= 450) {
                            ...
                            if (this.timeUntilTeleport < 446 && (this.posY <= 0D || this.posY <= (this.safeY - 16D))) {
            ...
            if (!this.worldObj.isRemote) {
                for (int k = 2; k < 23; k += 7) {
                    ...
                    if (state != BlocksAether.locked_ancient_angelic_stone
                        || state != BlocksAether.locked_ancient_light_angelic_stone) {
                        this.worldObj.setBlock(
                            this.dungeonX - 1,
                            this.dungeonY,
                            this.dungeonZ + k,
                            BlocksAether.locked_ancient_angelic_stone);
                        this.worldObj.setBlock(
                            this.dungeonX - 1,
                            this.dungeonY,
                            this.dungeonZ + k + 1,
    ...
    public void onUpdate() {
        ...
        if (!this.onGround && this.getEntityToAttack() != null
            ...
            && motionY < 0.0D
            && getDistanceToEntity(this.getEntityToAttack()) <= 16F
            && canEntityBeSeen(this.getEntityToAttack())) {
        ...
        if (!this.onGround && !isOnLadder()
            && Math.abs(this.motionY - this.lastMotionY) > 0.07D
            && Math.abs(this.motionY - this.lastMotionY) < 0.09D) {
            this.motionY += 0.055F;
        ...
        if (this.getHealth() <= 0 || this.isDead) {
            ...
            if (this.getEntityToAttack() instanceof EntityPlayer) {
                this.chatItUp(
                    (EntityPlayer) this.getEntityToAttack(),
                    StatCollector.translateToLocal("gui.queen.defeated"));
        ...
        if (ds.getEntity() instanceof EntityPlayer player) {
            ...
            if (!this.isBossReady()) {
                ...
                if (this.getEntityToAttack() == null) {
                    ...
                    if (ds.getEntity() instanceof EntityLivingBase) becomeAngryAt((EntityLivingBase) ds.getEntity());
                } else {
    ...
    public void teleportToPlayer() {
        if (this.getEntityToAttack() instanceof EntityPlayer) {
            ...
            this.enhancedCombat.resetTask();
            this.setPosition(
                this.getEntityToAttack().posX + 0.5D,
                this.getEntityToAttack().posY + 0.5D,
                this.getEntityToAttack().posZ + 0.5D);
