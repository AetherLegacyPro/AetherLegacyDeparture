...
public class EntityDivineSunSpirit extends EntityFlying implements IMob, IAetherBoss, IEntityMultiPart {
    ...
    public void makeFireBall(int var1) {
        ...
        for (int var3 = 0; var3 < var1; ++var3) {
            EntityCrystal crystal = new EntityCrystal(
                this.worldObj,
                this.posX - this.motionX / 2.4D,
                this.posY,
                this.posZ - this.motionZ / 2.0D,
                shootIceyBall ? EnumCrystalType.ICE : EnumCrystalType.HELLFIRE);
        ...
        for (int var3 = 0; var3 < var1; ++var3) {
            EntityCrystal crystal = new EntityCrystal(
                ...
                shootIceyBall ? EnumCrystalType.HELLFIRE : EnumCrystalType.HELLFIRE);
        ...
        for (x = this.originPointX - 20; x < this.originPointX + 20; ++x) {
            for (y = this.originPointY - 3; y < this.originPointY + 6; ++y) {
                for (z = this.originPointZ - 20; z < this.originPointZ + 20; ++z) {
                    ...
                    if (unlock_block == BlocksAether.locked_divine_hellfire_stone
                        || unlock_block == BlocksAether.locked_divine_light_hellfire_stone) {
    ...
    public String getBossName() {
        return this.dataWatcher.getWatchableObjectString(20) + ", "
            + StatCollector.translateToLocal("title.aether_legacy.divine_sun_spirit.name");
    }
