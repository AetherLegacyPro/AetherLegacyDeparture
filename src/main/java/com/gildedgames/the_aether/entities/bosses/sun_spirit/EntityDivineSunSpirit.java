...
public class EntityDivineSunSpirit extends EntityFlying implements IMob, IAetherBoss, IEntityMultiPart {
    ...
    public EntityDivineSunSpirit(World worldIn) {
        super(worldIn);
    ...
    public EntityDivineSunSpirit(World worldIn, int posX, int posY, int posZ, int var6) {
        this(worldIn);
    ...
    public void burnEntities() {
        ...
        for (int ammount = 0; ammount < entityList.size(); ++ammount) {
            ...
            if (entity instanceof EntityLivingBase && !entity.isImmuneToFire()) {
                ...
                entity.attackEntityFrom(new EntityDamageSource("incineration_firo", this), 20);
            }
        }
    }
    ...
    public void makeFireBall(int var1) {
        ...
        if (this.ballCount >= 1 + this.rand.nextInt(8)) {
            shootIceyBall = true;
            this.ballCount = 0;
        }

        for (int var3 = 0; var3 < var1; ++var3) {
            EntityCrystal crystal = new EntityCrystal(this.worldObj, this.posX - this.motionX / 2.4D, this.posY, this.posZ - this.motionZ / 2.0D, shootIceyBall ? EnumCrystalType.ICE : EnumCrystalType.HELLFIRE);
        ...
        for (int var3 = 0; var3 < var1; ++var3) {
            EntityCrystal crystal = new EntityCrystal(this.worldObj, this.posX - this.motionX / 2.4D, this.posY, this.posZ - this.motionZ / 2.2D, shootIceyBall ? EnumCrystalType.ICE : EnumCrystalType.HELLFIRE);
        ...
        for (int var3 = 0; var3 < var1; ++var3) {
            EntityCrystal crystal = new EntityCrystal(this.worldObj, this.posX - this.motionX / 2.4D, this.posY, this.posZ - this.motionZ / 1.8D, shootIceyBall ? EnumCrystalType.HELLFIRE : EnumCrystalType.HELLFIRE);
        ...
        for (int var3 = 0; var3 < var1; ++var3) {
            EntityCrystal crystal = new EntityCrystal(this.worldObj, this.posX - this.motionX / 2.4D, this.posY, this.posZ - this.motionZ / 1.8D, shootIceyBall ? EnumCrystalType.ICE : EnumCrystalType.HELLFIRE);
    ...
    public void summonFire() {
        ...
        if (this.worldObj.isAirBlock(x, y, z)) {
            this.worldObj.setBlock(x, y, z, BlocksAether.hellfire);
        }
    }
    ...
    public boolean interact(EntityPlayer player) {
        if (this.chatWithMe(player)) {
            ...
            this.setDoor(BlocksAether.locked_divine_hellfire_stone);
    ...
    {
        return this.attackEntityFrom(magicDamage, 20);
    }

    @Override
    protected void dropFewItems(boolean var1, int var2) {
        //this.entityDropItem(new ItemStack(ItemsAether.dungeon_key, 1, 2), 0.5F);
        this.entityDropItem(new ItemStack(ItemsAether.dungeon_key, 1, 8), 0.5F);
        
        int rand2 = (int)(1 + Math.random() * 2);
		switch (rand2)
        {
        case 1: this.dropItem(ItemsAether.divine_essence, 2 + rand.nextInt(3));
        break;
        case 2: 
        break;
        }
        ...
        for (x = this.originPointX - 20; x < this.originPointX + 20; ++x) {
            for (y = this.originPointY - 3; y < this.originPointY + 6; ++y) {
                for (z = this.originPointZ - 20; z < this.originPointZ + 20; ++z) {
                    ...
                    if (unlock_block == BlocksAether.locked_divine_hellfire_stone || unlock_block == BlocksAether.locked_divine_light_hellfire_stone) {
    ...
    public String getBossName() {
        return this.dataWatcher.getWatchableObjectString(20) + ", " + StatCollector.translateToLocal("title.aether_legacy.divine_sun_spirit.name");
    }
