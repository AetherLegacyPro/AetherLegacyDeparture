...
public class EntitySheepuff extends EntityAetherAnimal {
    ...
    public boolean getCanSpawnHere() {
        ...
        return (this.worldObj.getBlock(i, j - 1, k) == BlocksAether.aether_dirt
            ...
            && canSpawn
            && this.rand.nextInt(AetherConfig.getSheepuffSpawnrate()) == 0
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (itemstack != null
            && (itemstack.getItem() == Items.shears || itemstack.getItem() == ItemsAether.zanite_shears)
            && !this.getSheared()) {
            if (!this.worldObj.isRemote) {
                ...
                for (int j = 0; j < i; j++) {
                    EntityItem entityitem = this
                        .entityDropItem(new ItemStack(Blocks.wool, 1, this.getFleeceColor()), 1.0F);
        ...
        if (itemstack != null && (itemstack.getItem() == ItemsAether.shears_of_agnes) && !this.getSheared()) {
            if (!this.worldObj.isRemote) {
                ...
                for (int j = 0; j < i; j++) {
                    EntityItem entityitem = this
                        .entityDropItem(new ItemStack(Blocks.wool, 2, this.getFleeceColor()), 1.0F);
                    entityitem.motionY += this.rand.nextFloat() * 0.05F;
                    entityitem.motionX += (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F;
                    entityitem.motionZ += (this.rand.nextFloat() - this.rand.nextFloat()) * 0.1F;
                }
