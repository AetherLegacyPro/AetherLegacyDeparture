...
public class EntitySwet extends EntityMountable {
                        ...
                        if (!player.capabilities.isCreativeMode) {
                            ...
                            if (this.getType() == EnumSwetType.POISON) {
                                ((EntityLivingBase) entityIn)
                                    .addPotionEffect(new EffectInebriation(Potion.poison.id, 240, 0));
                            }
                        }
                    }
                }
    ...
    public boolean isPlayerFriendly(EntityPlayer player) {
        ...
        return iPlayerAether.getAccessoryInventory()
            .wearingAccessory(ItemsAether.swet_cape);
    }
    ...
    protected void dropFewItems(boolean recentlyHit, int lootLevel) {
        ...
        if (this.getType() == EnumSwetType.PURPLE) {
            ...
            switch (rand) {
                case 1:
                    this.entityDropItem(new ItemStack(BlocksAether.purple_aercloud, count), 1.0F);
                    break;
                case 2:
                    break;
                case 3:
