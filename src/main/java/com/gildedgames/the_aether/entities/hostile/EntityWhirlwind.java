...
public class EntityWhirlwind extends EntityMob {
    ...
    public float getBlockPathWeight(int x, int y, int z) {
        return this.worldObj.getBlock(x, y - 1, z) == BlocksAether.aether_grass ? 10.0F
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue((this.rand.nextDouble() * 0.025D) + 0.025D);
    ...
    public void onLivingUpdate() {
        ...
        if (this.getAttackTarget() == null) {
            this.motionX = Math.cos(0.01745329F * this.movementAngle)
                * this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .getAttributeValue();
            this.motionZ = -Math.sin(0.01745329F * this.movementAngle)
                * this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .getAttributeValue();
        ...
        if (!this.worldObj.isRemote) {
            ...
            if (this.actionTimer >= 128) {
                if (this.isEvil()) {
                    EntityUro entitycreeper = new EntityUro(this.worldObj);

                    entitycreeper.setLocationAndAngles(
                        this.posX,
                        this.posY + 0.5D,
                        this.posZ,
                        this.rand.nextFloat() * 360F,
                        0.0F);
        ...
        List<?> list = this.worldObj
            .getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(2.5D, 2.5D, 2.5D));
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (heldItem != null && heldItem.getItem() == Items.dye
            && AetherRankings.isRankedPlayer(player.getUniqueID())) {
    ...
    public void updateParticles() {
        if (!this.isEvil()) {
            ...
            for (int k = 0; k < 2; k++) {
                ...
                AetherParticle particle = new ParticlePassiveWhirly(
                    this.worldObj,
                    -Math.sin(0.01745329F * f) * 0.75D,
                    d4 - 0.25D,
                    Math.cos(0.01745329F * f) * 0.75D,
                    d1,
                    0.125D,
                    d7);
                FMLClientHandler.instance()
                    .getClient().effectRenderer.addEffect(particle);
                ...
                if (this.isRainbow) {
                    ...
                    particle.setRBGColorF(
                        EntitySheep.fleeceColorTable[l][0] * (1.0F - f1) + EntitySheep.fleeceColorTable[i1][0] * f1,
                        EntitySheep.fleeceColorTable[l][1] * (1.0F - f1) + EntitySheep.fleeceColorTable[i1][1] * f1,
                        EntitySheep.fleeceColorTable[l][2] * (1.0F - f1) + EntitySheep.fleeceColorTable[i1][2] * f1);
                }
            ...
            for (int k = 0; k < 3; k++) {
                ...
                AetherParticle particle = new ParticleEvilWhirly(
                    this.worldObj,
                    -Math.sin(0.01745329F * f1) * 0.75D,
                    d5 - 0.25D,
                    Math.cos(0.01745329F * f1) * 0.75D,
                    d2,
                    0.125D,
                    d8,
                    3.5F);
        ...
        if (!this.particles.isEmpty()) {
            for (int i1 = 0; i1 < this.particles.size(); i1++) {
                ...
                if (particle.isDead) {
                    ...
                    d21 += 160D;
                    particle
                        .setMotionX(-Math.cos(0.01745329424738884D * d21) * (d16 * 2.5D - d18) * 0.10000000149011612D);
                    particle
                        .setMotionZ(Math.sin(0.01745329424738884D * d21) * (d16 * 2.5D - d18) * 0.10000000149011612D);
                }
            }
        }
    }
    ...
    public boolean getCanSpawnHere() {
        ...
        return this.rand.nextInt(AetherConfig.getWhirlwindSpawnrate()) == 0
            && this.worldObj.getBlock(i, j - 1, k) == BlocksAether.aether_grass
            && this.worldObj.getBlockLightValue(i, j, k) > 8
