...
public class EntityYoungZephyr extends EntityAetherIIMob implements IMob, IFlyingMob {
    ...
    public void onLivingUpdate() {
        ...
        if (this.getAttackTimeSecs() < this.attackCooldown + this.rand.nextInt(60) && this.isAttacking()) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                this.setRotationYaw(
                    (float) Math.atan2(this.flyingAI.targetZ - this.posZ, this.flyingAI.targetX - this.posX));
            }
            this.motionX = 0.0;
            this.motionY = 0.0;
            this.motionZ = 0.0;
            ...
            if (this.getAttackTimeSecs() >= 40) {
                if (this.worldObj.isRemote) {
                    ...
                    final List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(
                        this,
                        this.boundingBox.addCoord(d5 / range2, d6 / range2, d7 / range2)
                            .expand(0.5, 0.1, 0.5));
        ...
        if (this.isAttacking()) {
            if (!this.puffedUp) {
                this.worldObj.playSoundAtEntity(
                    this,
                    "aether_legacy:aemob.zephyr.puff",
                    this.getSoundVolume(),
                ...
                this.worldObj.playSoundAtEntity(
                    this,
                    "aether_legacy:aemob.zephyr.blow",
                    this.getSoundVolume() * 0.6f,
                    ((this.rand.nextFloat() - this.rand.nextFloat()) * 0.2f + 1.0f) / 0.8f);
            }
        } else {
            this.puffedUp = false;
    ...
    public boolean getCanSpawnHere() {
        ...
        return this.worldObj.getBlock(i, j - 1, k) == BlocksAether.aether_grass
            && this.rand.nextInt(AetherConfig.getVulturnusSpawnrate()) == 0
            ...
            && this.worldObj.getBlockLightValue(
                ...
                MathHelper.floor_double(this.posZ)) > 7
    ...
    public void moveEntityWithHeading(final float par1, final float par2) {
        if (this.isInWater()) {
            ...
            if (this.onGround) {
                f2 = 0.54600006f;
                final Block i = this.worldObj.getBlock(
                    MathHelper.floor_double(this.posX),
            ...
            if (this.onGround) {
                f2 = 0.54600006f;
                final Block j = this.worldObj.getBlock(
                    MathHelper.floor_double(this.posX),
                    MathHelper.floor_double(this.boundingBox.minY) - 1,
                    MathHelper.floor_double(this.posZ));
                if (j != Blocks.air) {
