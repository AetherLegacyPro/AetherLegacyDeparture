...
public class EntityMimic extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(0.84000000417232513D);
                    ...
                    if (!player.capabilities.isCreativeMode) {
                        ...
                        if (this.getType() == EnumMimicType.RED) {
                            ...
                            ((EntityLivingBase) entityIn)
                                .addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 240, 1));
                        } else if (this.getType() == EnumMimicType.GREEN) {
                            this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                                .setBaseValue(10.0D);
                            ((EntityLivingBase) entityIn)
                                .addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 240, 1));
                        } else if (this.getType() == EnumMimicType.PURPLE) {
                            this.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                                .setBaseValue(10.0D);
                            ((EntityLivingBase) entityIn)
                                .addPotionEffect(new EffectInebriation(Potion.confusion.id, 240, 0));
                        }
                    }
                }
            }
    ...
    public boolean attackEntityFrom(DamageSource ds, float var2) {
        ...
        if (ds.getEntity() instanceof EntityPlayer player) {
            ...
            if (stack.getItem() instanceof ItemAetherTool
