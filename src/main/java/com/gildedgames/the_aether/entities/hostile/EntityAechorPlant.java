...
public class EntityAechorPlant extends EntityAetherAnimal {
    ...
    public void onLivingUpdate() {
        ...
        if (!this.isDead && this.getEntityToAttack() != null) {
            double distanceToPlayer = this.getEntityToAttack()
                .getDistanceToEntity(this);
        ...
        if (this.worldObj.getBlock(
            ...
            MathHelper.floor_double(this.posZ)) != BlocksAether.aether_grass) {
            this.setHealth(0.0F);
        }
    }
    ...
    public void shootAtPlayer() {
        ...
        double y = 0.1D + (Math.sqrt((x * x) + (z * z) + 0.1D) * 0.5D)
            + ((this.posY - this.getEntityToAttack().posY) * 0.25D);
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (heldItem != null && !this.worldObj.isRemote) {
            if (heldItem.getItem() == ItemsAether.skyroot_bucket
                ...
                if (--heldItem.stackSize == 0) {
                    player.setCurrentItemOrArmor(
                        0,
                        new ItemStack(ItemsAether.skyroot_bucket, 1, EnumSkyrootBucketType.Poison.meta));
    ...
    public boolean getCanSpawnHere() {
        ...
        return (this.worldObj.getBlock(i, j - 1, k) == BlocksAether.aether_dirt
            ...
            && canSpawn
            && this.rand.nextInt(AetherConfig.getAechorPlantSpawnrate()) == 0
