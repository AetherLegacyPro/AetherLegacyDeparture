...
public class EntityCyroGuardian extends EntityBossMob implements IAetherBoss {
    ...
    public void onEntityUpdate() {
        ...
        if (this.worldObj.isRemote) {
            for (int i = 0; i < 1; ++i) {
                NewAetherParticleHandler.CYRO.spawn(
                    worldObj,
                    this.posX + (this.rand.nextDouble() - 0.5D) * (double) this.width,
        ...
        if (this.attackTime <= 0 && p_70785_2_ < 2.0F
            ...
            if (this.attackTime == 0) {
                ...
                if (this.field_70846_g > 1) {
                    ...
                    for (int i = 0; i < 3; ++i) {
                        EntityCyroGuardianProjectile entitysmallfireball = new EntityCyroGuardianProjectile(
                            this.worldObj,
                            this,
                        ...
                        EntityCyroGuardianProjectile entitysmallfireball2 = new EntityCyroGuardianProjectile(
                            this.worldObj,
                            this,
                            d0 + this.rand.nextGaussian() * (double) f1,
                            d1,
    ...
    public String getBossName() {
        return this.dataWatcher.getWatchableObjectString(19) + ", "
            + StatCollector.translateToLocal("tile.aether_legacy.cyro_guardian.name");
    }
