    ...
    protected void updateEntityActionState() {
        ...
        if (this.getAttackTarget() == null) {
            ...
            if (this.getAttackTarget()
                .getDistanceSqToEntity(this) < 4096.0D && this.canEntityBeSeen(this.getAttackTarget())) {
                double x = this.getAttackTarget().posX - this.posX;
                double y = (this.getAttackTarget().boundingBox.minY + (this.getAttackTarget().height / 2.0F))
    ...
    protected void attackEntity(final Entity entity, final float f) {
        if (entity instanceof EntityLivingBase target) {
            if (f < 10.0f) {
                ...
                if (target != null) {
                    if (target.isDead || target.getDistanceToEntity(this) > 12.0
                        || target instanceof EntityZephyr
                        || target instanceof EntityTempest) {
                        target = null;
