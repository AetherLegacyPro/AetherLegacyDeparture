...
public class AIEntityFlyingMob extends EntityAIBase {
    ...
    public void updateTask() {
        ...
        this.flyingMob.getLookHelper()
            .setLookPosition(this.waypointX, this.waypointY, this.waypointZ, rotateTo, 0.0f);
        ...
        if (this.targetedEntity != null && (this.targetedEntity.isDead
            || Math.abs(this.flyingMob.posX - this.targetedEntity.posX) > this.distReqForTarget
            || Math.abs(this.flyingMob.posZ - this.targetedEntity.posZ) > this.distReqForTarget)) {
            this.targetedEntity = null;
        }
        if (this.targetedEntity == null || this.aggroCooldown-- <= 0) {
            final AxisAlignedBB bounds = AxisAlignedBB
                .getBoundingBox(
                    ...
                    this.flyingMob.posY,
                    this.flyingMob.posZ)
                .expand(this.distReqForTarget, 200.0, this.distReqForTarget);
    ...
    private boolean isCourseTraversable(final double d, final double d1, final double d2, final double d3) {
        ...
        for (int i = 1; i < d3; ++i) {
            ...
            if (!this.flyingMob.worldObj.getCollidingBoundingBoxes(this.flyingMob, axisalignedbb)
                .isEmpty()) {
                return false;
            }
        }
