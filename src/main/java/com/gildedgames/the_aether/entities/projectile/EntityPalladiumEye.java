...
public class EntityPalladiumEye extends Entity {
    ...
    public EntityPalladiumEye(World p_i1757_1_) {
        super(p_i1757_1_);
    ...
    public EntityPalladiumEye(World p_i1758_1_, double p_i1758_2_, double p_i1758_4_, double p_i1758_6_)
    {
        super(p_i1758_1_);
        this.despawnTimer = 0;
        this.setSize(0.25F, 0.25F);
    ...
    public void moveTowards(double p_70220_1_, int p_70220_3_, double p_70220_4_)
    {
        ...
        if (f > 12.0F)
        {
            this.targetX = this.posX + d2 / (double)f * 24.0D;
            this.targetZ = this.posZ + d3 / (double)f * 24.0D;
    ...
    public void onUpdate()
    {
        ...
        {
            this.worldObj.spawnParticle("enchantmenttable", this.posX - this.motionX * (double)f3 + this.rand.nextDouble() * 0.6D - 0.3D, this.posY - this.motionY * (double)f3 + 0.0D, this.posZ - this.motionZ * (double)f3 + this.rand.nextDouble() * 0.6D - 0.3D, this.motionX, this.motionY, this.motionZ);
        }
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (this.despawnTimer > 120 && !this.worldObj.isRemote)
            {
                this.setDead();

                if (this.shatterOrDrop)
                {
                    this.worldObj.spawnEntityInWorld(new EntityItem(this.worldObj, this.posX, this.posY, this.posZ, new ItemStack(ItemsAether.palladium_eye)));
                }
                else
                {
