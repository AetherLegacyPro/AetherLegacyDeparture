...
public class EntityCyroGuardianProjectile extends EntityFireball {
    ...
    public EntityCyroGuardianProjectile(World p_i1772_1_, double p_i1772_2_, double p_i1772_4_, double p_i1772_6_, double p_i1772_8_, double p_i1772_10_, double p_i1772_12_)
    {
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if(mop.entityHit instanceof EntityEnderman || mop.entityHit instanceof EntityCyro || mop.entityHit instanceof EntityCyroGuardian) this.setDead();

        if (!this.worldObj.isRemote) {
            if (mop.entityHit != null && mop.entityHit != this.shootingEntity) {
                mop.entityHit.attackEntityFrom(DamageSource.causeFireballDamage(this, this.shootingEntity), 3.0F);
                if (mop.entityHit instanceof EntityLivingBase) {
                    EntityLivingBase living = (EntityLivingBase)mop.entityHit;
                    living.addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 200, 0));
                    living.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 200, 0));
                }
            } else {
                int i = mop.blockX;
                int j = mop.blockY;
    ...
    public void onEntityUpdate() {
        ...
        if (this.worldObj.isRemote) {
            int k;
            for (k = 0; k < 4; ++k) {
                NewAetherParticleHandler.ETHEREAL_FLAME.spawn(worldObj, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height - 0.25D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width);
            }
        }
    }
