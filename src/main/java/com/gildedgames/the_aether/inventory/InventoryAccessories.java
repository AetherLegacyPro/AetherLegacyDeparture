...
public class InventoryAccessories implements IInventory, IAccessoryInventory {
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        ...
        if (stack != null && stack.getItem() instanceof ItemAccessory accessory) {
            return (orderedList.func_148745_a(slot) == accessory.getType()
                || (orderedList.func_148745_a(slot) == accessory.getExtraType())) && getStackInSlot(slot) == null;
        }
        return false;
    }
    ...
    public void damageWornItemsAtRate(DegradationRate degradationrate) {
        for (int slot = 0; slot < stacks.size(); ++slot) {
            ItemStack stack = stacks.get(slot);
            if (stack != null && stack.getItem() instanceof ItemAccessory accessory
                && accessory.getDegradationRate() == degradationrate
                && accessory.getType()
                    .degrades()) {
    ...
    public boolean wearingArmor(Item item) {
        if (item instanceof ItemArmor armor) {
            ItemStack armorStack = playerAether.getEntity()
                .getCurrentArmor(3 - armor.armorType);
