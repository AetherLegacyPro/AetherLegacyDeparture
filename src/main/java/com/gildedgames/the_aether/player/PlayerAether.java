...
public class PlayerAether implements IPlayerAether {
    ...
    public PlayerAether() {
        ...
        accessories = AetherConfig.UseBaublesExpandedMenu() ? new InventoryBaubles(this)
            : new InventoryAccessories(this);
    }
    ...
    public void onUpdate() {
        ...
        for (int i = 0; i < this.getAbilities()
            .size(); ++i) {
        ...
        boolean hasJumped = ReflectionHelper
            .getPrivateValue(EntityLivingBase.class, this.getEntity(), "isJumping", "field_70703_bu");
        ...
        if (this.inPortal) {
                ...
                if (this.getEntity().ridingEntity == null) {
                    if (this.portalCounter >= limit) {
                        ...
                        if (!this.getEntity().worldObj.isRemote) {
                            ...
                            if (this.player.worldObj.isRemote
                                && Minecraft.getMinecraft().thePlayer.getEntityId() == this.player.getEntityId()) {
                                Minecraft.getMinecraft().thePlayer
                                    .playSound("aether_legacy:aeportal.aetravel", 1.0f, 1.0f);
                            }
                            this.getEntity()
            ...
            if (this.getEntity().worldObj
                ...
                if (this.getEntity().worldObj
                    .getBlock((int) playerBounding.minX, (int) playerBounding.minY, (int) playerBounding.minZ)
                    != BlocksAether.aether_portal
                    && this.getEntity().worldObj
                        .getBlock((int) playerBounding.minX, (int) playerBounding.minY, (int) playerBounding.minZ)
                        != BlocksAether.aether_portal) {
                    this.inPortal = false;
                }
        ...
        if (!player.worldObj.isRemote) {
            if (this.bedLocation != null) {
                if (player.dimension == AetherConfig.getAetherDimensionID()) {
                    if (player.worldObj.getBlock(this.bedLocation.posX, this.bedLocation.posY, this.bedLocation.posZ)
    ...
    private void teleportPlayer(boolean shouldSpawnPortal) {
        if (this.getEntity() instanceof EntityPlayerMP) {
            ...
            int transferDimension = previousDimension == AetherConfig.getAetherDimensionID()
            ...
            if (server != null && server.getConfigurationManager() != null) {
                server.getConfigurationManager()
                    .transferPlayerToDimension((EntityPlayerMP) this.getEntity(), transferDimension, teleporter);
            }
        }
    }
    ...
    public void updatePowerShardCount(int amount) {
        ...
        if (!this.getEntity().worldObj.isRemote) {
            if (this.getPowerShardsUsed() <= this.getMaxPowerShardCount()) {
                ...
                if (this.player.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                    .getModifier(this.uuid) != null) {
                    this.player.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                        .removeModifier(this.powerModifier);
                }

                this.player.getEntityAttribute(SharedMonsterAttributes.attackDamage)
                    .applyModifier(this.powerModifier);
            } else {
                AetherNetwork.sendToAll(new PacketUpdatePowerShardCount(this.player, this.powerCount));
            }
        }
    ...
    public void updateDexShardCount(int amount) {
        ...
        if (!this.getEntity().worldObj.isRemote) {
            if (this.getDexShardsUsed() <= this.getMaxDexShardCount()) {
                ...
                if (this.player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .getModifier(this.uuid) != null) {
                    this.player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                        .removeModifier(this.dexModifier);
                }

                this.player.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                    .applyModifier(this.dexModifier);
            } else {
                AetherNetwork.sendToAll(new PacketUpdateDexterityShardCount(this.player, this.dexCount));
            }
        }
