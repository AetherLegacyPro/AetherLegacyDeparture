...
public class PlayerAetherEvents {
        ...
        if (!event.player.worldObj.isRemote) {
            ...
            if (event.player.worldObj.isRemote) {
                if (AetherConfig.enable_assets_message) {
                    ...
                    else player.addChatMessage(new ChatComponentText(I18n.format("gui.enable_assets2")));
    ...
    public void onPlayerAetherDrops(LivingDropsEvent event) {
        if (event.entityLiving instanceof EntityPlayer && !event.entityLiving.worldObj.getGameRules()
            .getGameRuleBooleanValue("keepInventory")) {
    ...
    public void onLivingHurt(LivingHurtEvent event) {
        ...
        // Other accessories
        if (accessoryInventory.wearingAccessory(ItemsAether.divineral_ring)
            || accessoryInventory.wearingAccessory(ItemsAether.divineral_pendant)) {
        ...
        if (Loader.isModLoaded("etfuturum") && AetherConfig.enableNetheriteRecipes()) {
            if (accessoryInventory.wearingAccessory(ItemsAether.netherite_ring)
                || accessoryInventory.wearingAccessory(ItemsAether.netherite_pendant)) {
    ...
    public void onLivingAttack(LivingAttackEvent event) {
        ...
        if (event.source.isFireDamage()) {
            if (accessoryInventory.isWearingPhoenixSet() || accessoryInventory.isWearingAmplifiedPhoenixSet()
        ...
        IAetherAbility ability = playerAether.getAbilities()
            .get(3);
    ...
    public void onUpdateBreakSpeed(BreakSpeed event) {
        PlayerAether.get(event.entityPlayer)
            .getAccessoryInventory()
            .getCurrentPlayerStrVsBlock(event.newSpeed);
    }
    ...
    public void onAchievementGet(AchievementEvent event) {
        ...
        int achievementType = achievement == AchievementsAether.defeat_bronze ? 1
            : achievement == AchievementsAether.defeat_silver ? 2 : 0;

        if (!player.worldObj.isRemote && ((EntityPlayerMP) player).func_147099_x()
            .canUnlockAchievement(achievement)
            ...
            if (event.achievement == AchievementsAether.enter_aether) {
                ...
                if (!player.inventory.addItemStackToInventory(loreBookStack)) {
                    player.worldObj.spawnEntityInWorld(
                        new EntityItem(player.worldObj, player.posX, player.posY, player.posZ, loreBookStack));
                }
