...
public class AbilityGravititeShield implements IAetherAbility {
	...
	public boolean shouldExecute() {
		return this.player.getAccessoryInventory().wearingAccessory(new ItemStack(ItemsAether.gravitite_shield));
	}
	...
	public void onUpdate() {
		...
		for (int size = 0; size < entities.size(); ++size) {
			...
			if (isProjectile(projectile) && this.getShooter(projectile) != this.player.getEntity()) {
				...
				this.player.getAccessoryInventory().damageWornStack(1, new ItemStack(ItemsAether.gravitite_shield));
			}
		}
	}
