...
public class AbilityAgilityBootsAndCape implements IAetherAbility {

    private final IPlayerAether player;

    private boolean stepUpdate;

    public AbilityAgilityBootsAndCape(IPlayerAether player) {
        this.player = player;
    }
    ...
    public void onUpdate() {
        ...
        if ((this.player.getAccessoryInventory()
            .wearingArmor(ItemsAether.agility_boots)
            || this.player.getAccessoryInventory()
                .wearingArmor(ItemsAether.scaled_agility_boots))
            && this.player.getAccessoryInventory()
            ...
            if (entityLiving.isEntityAlive()) {
                entityLiving.moveFlying(movementLR, movementFB, 0.018F);
            }
