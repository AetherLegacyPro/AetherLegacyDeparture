...
public class AbilityAccessories implements IAetherAbility {
    ...
    public void onUpdate() {
        ...
        if (!playerEntity.worldObj.isRemote && (accessoryInventory.wearingAccessory(ItemsAether.ice_ring)
            || accessoryInventory.wearingAccessory(ItemsAether.ice_pendant))) {
            ...
            for (int x = i - 1; x <= i + 1; x++) {
                for (int y = j - 1; y <= j + 1; y++) {
                    for (int z = k - 1; z <= k + 1; z++) {
                        ...
                        Block setBlock = (block == Blocks.water || block == Blocks.flowing_water) ? Blocks.ice
                            : (block == Blocks.lava || block == Blocks.flowing_lava) ? Blocks.obsidian : null;
        ...
        if (accessoryInventory.wearingAccessory(ItemsAether.iron_bubble)
            || accessoryInventory.wearingAccessory(ItemsAether.reinforced_iron_bubble)
            || accessoryInventory.wearingAccessory(ItemsAether.amplified_iron_bubble)) {
        ...
        } else if (!accessoryInventory.wearingAccessory(ItemsAether.invisibility_cape)
            && !playerEntity.isPotionActive(Potion.invisibility)) {
        ...
        if (accessoryInventory.wearingAccessory(ItemsAether.regeneration_stone)
            || accessoryInventory.wearingAccessory(ItemsAether.reinforced_regeneration_stone)) {
            if (playerEntity.getHealth() < playerEntity.getMaxHealth()
                && playerEntity.getActivePotionEffect(Potion.regeneration) == null) {
        ...
        if (ticksExisted % 5 == 0 && playerEntity.isWet()) {
            ...
            accessoryInventory
                .damageWornItem(1, ItemsAether.amplified_phoenix_gloves, ItemsAether.amplified_obsidian_gloves);
        }

        if (accessoryInventory.wearingAccessory(ItemsAether.golden_feather)
            || accessoryInventory.wearingAccessory(ItemsAether.reinforced_golden_feather)
            || accessoryInventory.wearingAccessory(ItemsAether.amplified_golden_feather)
            || player.getAccessoryInventory()
                .wearingAccessory(ItemsAether.valkyrie_cape)) {
            if (!playerEntity.onGround && playerEntity.motionY < 0.0D
