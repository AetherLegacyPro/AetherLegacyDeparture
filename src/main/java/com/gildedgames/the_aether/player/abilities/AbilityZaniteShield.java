...
public class AbilityZaniteShield implements IAetherAbility {
    ...
    public AbilityZaniteShield(IPlayerAether player) {
        this.player = player;
    }

    @Override
    public boolean shouldExecute() {
        return player.getAccessoryInventory()
            .wearingAccessory(ItemsAether.zanite_shield);
    }
    ...
    public void onUpdate() {
        ...
        List<?> entities = this.player.getEntity().worldObj.getEntitiesWithinAABBExcludingEntity(
            this.player.getEntity(),
            this.player.getEntity().boundingBox.expand(3.0D, 3.0D, 3.0D));
        ...
        for (int size = 0; size < entities.size(); ++size) {
            ...
            if (isProjectile(projectile) && this.getShooter(projectile) != this.player.getEntity()) {
                ...
                ((WorldServer) this.player.getEntity().worldObj).func_147487_a(
                    "flame",
                    projectile.posX,
                    projectile.posY,
                    projectile.posZ,
                ...
                this.player.getAccessoryInventory()
                    .damageWornItem(1, ItemsAether.zanite_shield);
            }
        }
    }
    ...
    private Entity getShooter(Entity ent) {
        return ent instanceof EntityArrow ? ((EntityArrow) ent).shootingEntity
            : ent instanceof EntityThrowable ? ((EntityThrowable) ent).getThrower()
                : ent instanceof EntityProjectileBase ? ((EntityProjectileBase) ent).getThrower()
                    : ent instanceof EntityFireball ? ((EntityFireball) ent).shootingEntity : null;
    }
