...
public class AbilityArkeniumArmor implements IAetherAbility {
    ...
    public void onUpdate() {
        ...
        if (this.player.getAccessoryInventory()
            .isWearingArkeniumSet()) {

            this.player.getEntity()
                .addPotionEffect(new EffectInebriation(Potion.nightVision.id, 300, 0));

        }

        if (this.player.getAccessoryInventory()
            .isWearingArkeniumComboSet()) {
        ...
        if (this.player.getAccessoryInventory()
            .isWearingAmplifiedArkeniumSet()) {

            this.player.getEntity()
                .addPotionEffect(new EffectInebriation(Potion.nightVision.id, 300, 0));
            this.player.getEntity()
                .addPotionEffect(new EffectInebriation(Potion.digSpeed.id, 20, 0));
