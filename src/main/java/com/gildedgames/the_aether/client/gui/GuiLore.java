    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (searchedStack != null) {
            if (this.currentItem == null
                || (searchedStack.getItem() != this.currentItem.getItem() || (!searchedStack.isItemStackDamageable()
                    && searchedStack.getItemDamage() != this.currentItem.getItemDamage()))) {
                this.pageNumber = 0;
            ...
            for (String name : this.fontRendererObj.listFormattedStringToWidth(
                searchedStack.getItem()
                    .getItemStackDisplayName(searchedStack),
                109)) {
            ...
            for (String lore : this.fontRendererObj.listFormattedStringToWidth(this.stringToLoad, 109)) {
                if (size >= 15 * this.pageNumber && size + (nameSize - 1) <= 15 * (this.pageNumber + 1)) {
                    int actualSize = this.pageNumber >= 1 ? size - ((15 - (nameSize - 1)) * this.pageNumber)
                        : size + (nameSize - 1);

                    this.fontRendererObj.drawString(
                        lore,
                        (((actualSize >= 6 ? 184 : 71)) - this.fontRendererObj.getStringWidth(lore) / 2),
