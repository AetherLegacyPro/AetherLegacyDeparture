...
public class AetherClientEvents {
                ...
                if (Mouse.getEventButtonState()) {
                    ...
                    if (stack != null) {
                        if (isValkyrieItem(stack.getItem())) {
                            ...
                            List<Entity> locatedEntities = player.worldObj
                                .getEntitiesWithinAABB(Entity.class, reachDistance);
                            ...
                            for (Entity ent : locatedEntities) {
                                ...
                                Vec3 vec = Vec3.createVectorHelper(
                                    ...
                                    ent.boundingBox.minY + ent.height / 2f - player.posY - player.getEyeHeight(),
                            ...
                            List<Entity> locatedEntitiess = player.worldObj
                                .getEntitiesWithinAABB(Entity.class, reachDistancee);
                            ...
                            for (Entity entt : locatedEntitiess) {
                                ...
                                Vec3 vecc = Vec3.createVectorHelper(
                                    entt.posX - player.posX,
                                    entt.boundingBox.minY + entt.height / 2f - player.posY - player.getEyeHeight(),
                                    entt.posZ - player.posZ);
    ...
    public boolean isValkyrieItem(Item item) {
        return item == ItemsAether.valkyrie_shovel || item == ItemsAether.valkyrie_axe
            || item == ItemsAether.valkyrie_pickaxe
            || item == ItemsAether.valkyrie_lance
            || item == ItemsAether.tipped_valkyrie_lance
            || item == ItemsAether.tipped_valkyrie_pickaxe
            || item == ItemsAether.tipped_valkyrie_shovel
            || item == ItemsAether.tipped_valkyrie_axe;
    }

    public boolean isAmplifiedValkyrieItem(Item stackIDD) {
        return stackIDD == ItemsAether.amplified_valkyrie_shovel || stackIDD == ItemsAether.amplified_valkyrie_axe
            || stackIDD == ItemsAether.amplified_valkyrie_pickaxe
            || stackIDD == ItemsAether.amplified_valkyrie_lance;
    }

    public boolean isAscensiteItem(Item stackIDD) {
        return stackIDD == ItemsAether.ascensite_shovel || stackIDD == ItemsAether.ascensite_axe
            || stackIDD == ItemsAether.ascensite_pickaxe
            || stackIDD == ItemsAether.ascensite_sword;
    }

    public boolean isArkeniumItem(Item stackIDDA) {
        return stackIDDA == ItemsAether.arkenium_shovel || stackIDDA == ItemsAether.arkenium_axe
            || stackIDDA == ItemsAether.arkenium_pickaxe
            || stackIDDA == ItemsAether.arkenium_sword
            || stackIDDA == ItemsAether.tipped_arkenium_sword
            || stackIDDA == ItemsAether.tipped_arkenium_pickaxe
            || stackIDDA == ItemsAether.tipped_arkenium_shovel
            || stackIDDA == ItemsAether.tipped_arkenium_axe;
    }

    public boolean isContinuumItem(Item stackIDDA) {
        return stackIDDA == ItemsAether.continuum_shovel || stackIDDA == ItemsAether.continuum_axe
            || stackIDDA == ItemsAether.continuum_pickaxe
            || stackIDDA == ItemsAether.continuum_sword
            || stackIDDA == ItemsAether.tipped_continuum_sword
            || stackIDDA == ItemsAether.tipped_continuum_pickaxe
            || stackIDDA == ItemsAether.tipped_continuum_shovel
            || stackIDDA == ItemsAether.tipped_continuum_axe;
    }

    public boolean isAmplifiedContinuumItem(Item stackIDDA) {
        return stackIDDA == ItemsAether.amplified_continuum_shovel || stackIDDA == ItemsAether.amplified_continuum_axe
            || stackIDDA == ItemsAether.amplified_continuum_pickaxe
            || stackIDDA == ItemsAether.amplified_continuum_sword;
    }
    ...
    private void sendPickupPacket(Minecraft mc) {
        if (mc.objectMouseOver != null) {
            ...
            if (mc.thePlayer.capabilities.isCreativeMode) {
                ...
                mc.playerController
                    .sendSlotPacket(mc.thePlayer.inventory.getStackInSlot(mc.thePlayer.inventory.currentItem), index);
            }
        }
    }
    ...
    public void onBowPulled(FOVUpdateEvent event) {
        ...
        if (item == ItemsAether.phoenix_bow || item == ItemsAether.tipped_phoenix_bow
            || item == ItemsAether.amplified_phoenix_bow
            || item == ItemsAether.cyro_bow
            || item == ItemsAether.tipped_cyro_bow
            || item == ItemsAether.amplified_cyro_bow) {
    ...
    public void onGuiOpened(GuiScreenEvent.InitGuiEvent.Post event) {
        if (event.gui instanceof GuiContainer) {
            ...
            int guiLeft = ObfuscationReflectionHelper
                .getPrivateValue(GuiContainer.class, (GuiContainer) event.gui, "guiLeft", "field_147003_i");
            int guiTop = ObfuscationReflectionHelper
                .getPrivateValue(GuiContainer.class, (GuiContainer) event.gui, "guiTop", "field_147009_r");
            int xSize = ObfuscationReflectionHelper
                .getPrivateValue(GuiContainer.class, (GuiContainer) event.gui, "xSize", "field_146999_f");
            int ySize = ObfuscationReflectionHelper
                .getPrivateValue(GuiContainer.class, (GuiContainer) event.gui, "ySize", "field_147000_g");
            ...
            if (!AetherConfig.UseBaublesExpandedMenu()) {
                if (player.capabilities.isCreativeMode) {
                    ...
                    // We place the buttons where they would be if the inventory was its original size, for consistency
                    // with Baubles's button placement algorithm
                    int xSizeOriginal = 176;
                    int ySizeOriginal = 166;
                    event.buttonList.add(
                        ACCESSORY_BUTTON.setPosition(
                            (xSize - xSizeOriginal) / 2 + guiLeft + 26,
                            (ySize - ySizeOriginal) / 2 + guiTop + 65));
                }
            }
        ...
        if (!openedMenu && AetherConfig.getMenuEnabled() && event.gui instanceof GuiMainMenu) {
            ...
            Minecraft.getMinecraft()
                .displayGuiScreen(new AetherMainMenu());
        }
        ...
        if (event.gui instanceof GuiOptions) {
            if (Minecraft.getMinecraft().thePlayer != null) {
                if (AetherRankings.isRankedPlayer(Minecraft.getMinecraft().thePlayer.getUniqueID())) {
                    event.buttonList.add(
                        new GuiCustomizationScreenButton(
                            545,
        ...
        if (event.gui instanceof ScreenChatOptions) {
            if (Minecraft.getMinecraft().thePlayer != null) {
                int i = 13;
                event.buttonList.add(
                    new GuiCapeButton(event.gui.width / 2 - 155 + i % 2 * 160, event.gui.height / 6 + 24 * (i >> 1)));
            }
        }
    }
    ...
    public void onMouseClicked(DrawScreenEvent.Post event) {
        if (Minecraft.getMinecraft().currentScreen instanceof GuiContainerCreative guiScreen
            && !AetherConfig.UseBaublesExpandedMenu()) {
            if (previousSelectedTabIndex != guiScreen.func_147056_g()) {
                List<GuiButton> buttonList = ObfuscationReflectionHelper
                    .getPrivateValue(GuiScreen.class, (GuiScreen) guiScreen, 4);

                if (guiScreen.func_147056_g() == CreativeTabs.tabInventory.getTabIndex()
                    && !buttonList.contains(ACCESSORY_BUTTON)) {
                    int guiLeft = ObfuscationReflectionHelper
                        .getPrivateValue(GuiContainer.class, (GuiContainer) guiScreen, "guiLeft", "field_147003_i");
                    int guiTop = ObfuscationReflectionHelper
                        .getPrivateValue(GuiContainer.class, (GuiContainer) guiScreen, "guiTop", "field_147009_r");
    ...
    public void onButtonPressed(GuiScreenEvent.ActionPerformedEvent.Pre event) {
        ...
        if (event.button instanceof GuiCustomizationScreenButton) {
            Minecraft.getMinecraft()
                .displayGuiScreen(new GuiCustomizationScreen(event.gui));
        }
    ...
    public void onRenderInvisibility(RenderPlayerEvent.Pre event) {
        ...
        PlayerAetherRenderer.instance()
            .setPartialTicks(event.partialRenderTick);
    }

    @SubscribeEvent
    public void onRenderAetherCape(RenderPlayerEvent.Specials.Pre event) {
        event.renderCape = !PlayerAetherRenderer.instance()
            .isCapeRendering();
    }
