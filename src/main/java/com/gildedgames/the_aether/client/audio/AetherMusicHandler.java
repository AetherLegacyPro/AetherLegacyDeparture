...
public class AetherMusicHandler {
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) throws Exception {
        ...
        if (!(mc.getSoundHandler()
            .isSoundPlaying(musicTicker.getRecord()))) {
            musicTicker.trackRecord(null);
        }

        if (AetherConfig.getMenuEnabled() && Minecraft.getMinecraft().theWorld == null
        ...
        if (category == SoundCategory.MUSIC) {
            if (this.mc.thePlayer != null && this.mc.thePlayer.dimension == AetherConfig.getAetherDimensionID()) {
                if (!sound.getPositionedSoundLocation()
                    .toString()
                    .contains("aether_legacy")
                    && (this.musicTicker.playingMusic() || !this.musicTicker.playingMusic())) {
                    event.result = null;
    ...
    public static ISound getAchievementSound(int number) {
        ResourceLocation sound = number == 1 ? Aether.locate("achievement_bronze")
            : number == 2 ? Aether.locate("achievement_silver") : Aether.locate("achievement");
