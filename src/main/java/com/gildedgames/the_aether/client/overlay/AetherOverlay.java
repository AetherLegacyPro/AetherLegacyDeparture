...
public class AetherOverlay {
    ...
    public static void renderIronBubble(Minecraft mc) {
        ...
        int bubbleAmount = PlayerAether.get(mc.thePlayer)
            .getAccessoryInventory()
            .getAccessoryCount(ItemsAether.iron_bubble);

        if (mc.playerController.shouldDrawHUD() && mc.thePlayer.isInWater()
        ...
        int bubbleAmount2 = PlayerAether.get(mc.thePlayer)
            .getAccessoryInventory()
            .getAccessoryCount(ItemsAether.reinforced_iron_bubble);
        ...
        int bubbleAmount3 = PlayerAether.get(mc.thePlayer)
            .getAccessoryInventory()
            .getAccessoryCount(ItemsAether.amplified_iron_bubble);
    ...
    public static void renderCooldown(Minecraft mc) {
        ...
        if (playerInfo.getHammerCooldown() != 0) {
            ...
            int cooldownRemaining = (int) ((float) (playerInfo.getHammerCooldown())
                / (float) (playerInfo.getHammerMaxCooldown())
                * 128F);
            int width = scaledresolution.getScaledWidth();

            mc.fontRenderer.drawStringWithShadow(
                playerInfo.getHammerName() + " " + I18n.format("item.notch_hammer.cooldown"),
                (width / 2) - (mc.fontRenderer.getStringWidth(playerInfo.getHammerName() + " Cooldown") / 2),
                32,
                0xffffffff);
    ...
    public static void renderBossHP(Minecraft mc) {
        ...
        if (player.getFocusedBoss() != null) {
            ...
            mc.fontRenderer.drawStringWithShadow(
                bossTitle,
                width / 2 - (mc.fontRenderer.getStringWidth(bossTitle) / 2),
                2,
                0xffffffff);
