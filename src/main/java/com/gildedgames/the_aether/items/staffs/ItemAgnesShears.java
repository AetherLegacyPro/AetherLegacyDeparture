...
public class ItemAgnesShears extends ItemShears {
    ...
    public ItemAgnesShears() {
        ...
        this.setMaxDamage(1038);
        this.setCreativeTab(AetherCreativeTabs.tools);
    }

    public boolean onBlockDestroyed(ItemStack stack, World world, Block block, int p_150894_4_, int p_150894_5_, int p_150894_6_, EntityLivingBase entityLivingBase) {
        if (block.getMaterial() != Material.leaves && block != Blocks.web && block != Blocks.tallgrass && block != Blocks.vine && block != Blocks.tripwire && block != BlocksAether.aether_tallgrass && block != BlocksAether.enchanted_aether_tallgrass && !(block instanceof IShearable)) {
            return super.onBlockDestroyed(stack, world, block, p_150894_4_, p_150894_5_, p_150894_6_, entityLivingBase);
        }
        else {
            return true;
        }
        ...
        if (entity instanceof IShearable target) {
			if (target.isShearable(itemstack, entity.worldObj, (int)entity.posX, (int)entity.posY, (int)entity.posZ)) {
                ...
                for(ItemStack stack : drops) {
                    ...
                    EntityItem ent2 = entity.entityDropItem(stack, 1.0F);
                    ent2.motionY += rand.nextFloat() * 0.05F;
                    ent2.motionX += (rand.nextFloat() - rand.nextFloat()) * 0.1F;
                    ent2.motionZ += (rand.nextFloat() - rand.nextFloat()) * 0.1F;
                }
