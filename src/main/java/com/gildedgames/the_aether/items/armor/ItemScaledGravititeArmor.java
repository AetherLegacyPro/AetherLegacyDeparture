...
public class ItemScaledGravititeArmor extends ItemArkeniumArmorBase implements ISpecialArmor {

    private final int[][] damageReductionAmountArray = new int[][] { new int[] { 1, 3, 2, 1 }, new int[] { 1, 5, 4, 2 },
        new int[] { 2, 6, 5, 2 }, new int[] { 3, 8, 6, 3 }, new int[] { 4, 10, 8, 4 } };

    public ItemScaledGravititeArmor(int armorType, ArmorMaterial material, String name, Item repair) {
    ...
    public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip,
        ...
        if (AetherConfig.enableTooltips())
            tooltip.add(EnumChatFormatting.AQUA + "" + StatCollector.translateToLocal("tooltip.gravitite_armor.desc"));
    }
