...
public class ItemAmplifiedCyroBow extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemAmplifiedCyroBow() {
		super();
		this.maxStackSize = 1;
		this.setMaxDamage(1588);
		this.setFull3D();
		this.setTextureName(Aether.find("weapons/amplified_cyro_bow"));
	...
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == ItemsAether.amplified_cyro_bow) {
	...
	public void registerIcons(IIconRegister iconRegister) {
		this.itemIcon = iconRegister.registerIcon(Aether.find("weapons/amplified_cyro_bow"));
		...
		if (entityLiving != null) {
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						EntityCyroArrow entityarrow = this.createArrow(worldIn, f * 2.0F, itemstack, entityplayer);
