...
public class ItemCyroBow extends ItemBow {
	...
	public ItemCyroBow() {
		...
		this.setFull3D();
		this.setTextureName(Aether.find("weapons/cyro_bow"));
	...
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == ItemsAether.cyro_bow) {
		...
		if (entityLiving != null) {
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						EntityCyroArrow entityarrow = this.createArrow(worldIn, f * 1.8F, itemstack, entityplayer);
