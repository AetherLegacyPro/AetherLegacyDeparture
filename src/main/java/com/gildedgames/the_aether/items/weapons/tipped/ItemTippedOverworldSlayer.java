...
public class ItemTippedOverworldSlayer extends ItemSword {
    ...
    public ItemTippedOverworldSlayer() {
        super(ToolMaterial.EMERALD);
        this.setMaxDamage(3001);
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(ItemStack stack) {
        ...
        if (stack.getItem() instanceof ItemTippedOverworldSlayer) {
            if (newWorld == true) {
                multimap.put(
                    SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(),
                    new AttributeModifier(field_111210_e, "Weapon modifier", this.calculateIncrease2(stack), 0));
            } else {
                multimap.put(
                    SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(),
    ...
    private float calculateIncrease2(ItemStack tool) {
        ...
        if (isBetween(tool.getMaxDamage(), current, tool.getMaxDamage() - 50)) {
            ...
            return level2[2];
        } else if (isBetween(tool.getMaxDamage() - 201, current, tool.getMaxDamage() - 3001)) {
            return level2[1];
        } else {
            return level2[0];
        }
