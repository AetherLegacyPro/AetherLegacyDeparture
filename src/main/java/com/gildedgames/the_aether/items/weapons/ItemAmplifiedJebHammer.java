    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(ItemStack stack) {
        ...
        if (stack.getItem() instanceof ItemAmplifiedJebHammer) {
            multimap.put(
                ...
                new AttributeModifier(field_111210_e, "Weapon modifier", this.calculateIncrease(stack), 0));
        }
    ...
    public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
        if (entityplayer.capabilities.isCreativeMode) {
            world.playSound(
                ...
                "mob.ghast.fireball",
                0.5F,
                0.5F / (itemRand.nextFloat() * 0.2F + 0.4F),
                false);

            if (!world.isRemote) {
                EntityAmplifiedJebHammerProjectile hammerProjectile = new EntityAmplifiedJebHammerProjectile(
                    world,
                    entityplayer);
                hammerProjectile
                    .shoot(entityplayer, entityplayer.rotationPitch, entityplayer.rotationYaw, 0.0F, 1.5F, 1.0F);
                world.spawnEntityInWorld(hammerProjectile);
            }
        } else if (PlayerAether.get(entityplayer)
            .setHammerCooldown(250, itemstack.getDisplayName())) {
                ...
                if (!world.isRemote) {
                    EntityAmplifiedJebHammerProjectile hammerProjectile = new EntityAmplifiedJebHammerProjectile(
                        ...
                        entityplayer);
                    hammerProjectile
                        .shoot(entityplayer, entityplayer.rotationPitch, entityplayer.rotationYaw, 0.0F, 1.3F, 1.0F);
                    world.spawnEntityInWorld(hammerProjectile);
                }
            }
