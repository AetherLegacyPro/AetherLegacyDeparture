...
public class ItemTippedCyroBow extends ItemBow {
	...
	public ItemTippedCyroBow() {
		...
		this.setFull3D();
		this.setTextureName(Aether.find("weapons/tipped_cyro_bow"));
	...
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == ItemsAether.tipped_cyro_bow) {
	...
	public void registerIcons(IIconRegister p_94581_1_) {
		this.itemIcon = p_94581_1_.registerIcon(Aether.find("weapons/tipped_cyro_bow"));
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						EntityCyroArrow entityarrow = this.createArrow(worldIn, f * 1.9F, itemstack, entityplayer);
