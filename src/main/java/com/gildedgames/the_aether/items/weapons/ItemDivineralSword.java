...
public class ItemDivineralSword extends ItemSword {

    public float[] level = new float[] { 9.0F, 9.0F, 9.0F, 9.0F, 9.0F };

    public ItemDivineralSword() {
    ...
    public Multimap<String, AttributeModifier> getAttributeModifiers(ItemStack stack) {
        ...
        if (stack.getItem() instanceof ItemDivineralSword) {
            multimap.put(
    ...
    public boolean hitEntity(ItemStack itemstack, EntityLivingBase hitentity, EntityLivingBase player) {
        if ((hitentity.hurtTime > 0 || hitentity.deathTime > 0) && !(hitentity instanceof EntitySwet)) {
            hitentity.addVelocity(0.0D, 1.1D, 0.0D);
        }
    ...
    public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip,
        ...
        if (AetherConfig.enableTooltips()) tooltip
            .add(EnumChatFormatting.LIGHT_PURPLE + "" + StatCollector.translateToLocal("tooltip.divineral_sword.desc"));
    }
