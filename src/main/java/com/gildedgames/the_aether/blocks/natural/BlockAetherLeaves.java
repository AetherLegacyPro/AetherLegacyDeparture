...
public class BlockAetherLeaves extends BlockLeaves {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (this == BlocksAether.golden_oak_leaves) {
            for (int ammount = 0; ammount < 2; ammount++) {
                ...
                FMLClientHandler.instance()
                    .getClient().effectRenderer.addEffect(obj);
            }
        }
        ...
        if (this == BlocksAether.crystal_leaves || this == BlocksAether.crystal_fruit_leaves
            || this == BlocksAether.divine_oak_leaves) {
            if (rand.nextInt(5) == 0) {
                for (int l = 0; l < 6; ++l) {
    ...
    public Item getItemDropped(int meta, Random random, int fortune) {
        return this == BlocksAether.skyroot_leaves ? Item.getItemFromBlock(BlocksAether.skyroot_sapling)
            : this == BlocksAether.golden_oak_leaves ? Item.getItemFromBlock(BlocksAether.golden_oak_sapling)
                : this == BlocksAether.blue_skyroot_leaves ? Item.getItemFromBlock(BlocksAether.blue_skyroot_sapling)
                    : this == BlocksAether.purple_skyroot_leaves
                        ? Item.getItemFromBlock(BlocksAether.purple_skyroot_sapling)
                        : this == BlocksAether.dark_blue_skyroot_leaves
                            ? Item.getItemFromBlock(BlocksAether.dark_blue_skyroot_sapling)
                            : this == BlocksAether.crystal_leaves ? Item.getItemFromBlock(BlocksAether.crystal_sapling)
                                : this == BlocksAether.blue_light_skyroot_leaves
                                    ? Item.getItemFromBlock(BlocksAether.wisproot_sapling)
                                    : this == BlocksAether.green_light_skyroot_leaves
                                        ? Item.getItemFromBlock(BlocksAether.greatroot_sapling)
                                        : this == BlocksAether.void_leaves
                                            ? Item.getItemFromBlock(BlocksAether.void_sapling)
                                            : null;
    }
