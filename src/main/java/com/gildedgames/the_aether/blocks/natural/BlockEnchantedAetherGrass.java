		...
		while (l < 128) {
			...
			while (true) {
				if (l1 < l / 16) {
					...
					if (world.getBlock(i1, j1 - 1, k1) == BlocksAether.enchanted_aether_grass && !world.getBlock(i1, j1, k1).isNormalCube()) {
						++l1;
						continue;
					}
				} else if (world.isAirBlock(i1, j1, k1)) {
					if (random.nextInt(8) != 0) {
						if (BlocksAether.enchanted_aether_tallgrass.canBlockStay(world, i1, j1, k1)) {
							world.setBlock(i1, j1, k1, BlocksAether.enchanted_aether_tallgrass, 1, 3);
						}
					}
					else if (random.nextInt(7) != 0) {
						if (BlocksAether.enchanted_aether_tulips.canBlockStay(world, i1, j1, k1)) {
							world.setBlock(i1, j1, k1, BlocksAether.enchanted_aether_tulips, 1, 3);
						}
					}
					else if (random.nextInt(6) != 0) {
						if (BlocksAether.enchanted_quickshoot.canBlockStay(world, i1, j1, k1)) {
							world.setBlock(i1, j1, k1, BlocksAether.enchanted_quickshoot, 1, 3);
						}
					}
				}
