...
public class BlockVoidSapling extends BlockUpsideDownFlower implements IGrowable {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (stack.getItem() == ItemsAether.swet_ball || stack.getItem() == ItemsAether.golden_swet_ball
            || stack.getItem() == ItemsAether.purple_swet_ball) {

            int rand = (int) (1 + Math.random() * 4);
            if (world.getBlockLightValue(x, y + 1, z) <= 7 && rand == 4
                && y <= 30.0D
                && world.provider.dimensionId == AetherConfig.getAetherDimensionID()) {
                world.setBlock(x, y, z, Blocks.air);
