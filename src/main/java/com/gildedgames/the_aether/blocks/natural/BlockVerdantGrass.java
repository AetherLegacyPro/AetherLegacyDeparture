...
public class BlockVerdantGrass extends Block implements IGrowable {
            ...
            if (worldIn.getBlockLightValue(x, y - 1, z) >= 9) {
                for (int l = 0; l < 4; ++l) {
                    ...
                    if (worldIn.getBlock(i1, j1, k1) == BlocksAether.aether_dirt
                        && (worldIn.getBlockMetadata(i1, j1, k1) == 0 || worldIn.getBlockMetadata(i1, j1, k1) == 1)) {
                        worldIn.setBlock(i1, j1, k1, BlocksAether.verdant_grass);
                    }
                }
            }
        }
    ...
    public void registerBlockIcons(IIconRegister registry) {
        this.blockIcon = registry.registerIcon("aether_legacy:verdant_grass_side");
        this.blockIconSnowy = registry.registerIcon("aether_legacy:aether_grass_side_snowy");
        this.blockIconTop = registry.registerIcon("aether_legacy:aether_dirt");
        this.blockIconBottom = registry.registerIcon(Aether.find("verdant_grass_top"));
    }
            ...
            while (true) {
                if (l1 < l / 16) {
                    ...
                    if (p_149853_2_.nextInt(8) != 0) {
                        if (BlocksAether.aether_tallgrass.canBlockStay(p_149853_1_, i1, j1, k1)) {
                            p_149853_1_.setBlock(i1, j1, k1, BlocksAether.aether_tallgrass, 1, 3);
                        }
