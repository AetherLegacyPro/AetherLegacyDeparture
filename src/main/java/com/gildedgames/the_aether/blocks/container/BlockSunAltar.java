...
public class BlockSunAltar extends Block {
    ...
    public IIcon getIcon(int side, int meta) {
        return side == 1 ? this.blockIconTop
            : (side == 0 ? BlocksAether.hellfire_stone.getBlockTextureFromSide(side) : this.blockIcon);
    }
        ...
        if (player.dimension == AetherConfig.getAetherDimensionID()) {
            if (world.provider instanceof AetherWorldProvider provider) {
                ...
                if (provider.getIsEternalDay() && provider.getShouldCycleCatchup()) {
                    if (server != null && ((server.isDedicatedServer() && (server.getConfigurationManager()
                        .func_152596_g(player.getGameProfile()) || AetherConfig.sunAltarMultiplayer())
                        || !server.isDedicatedServer()))) {
                        Aether.proxy.openSunAltar();
                    } else if (world.isRemote) {
                        if (player instanceof EntityPlayerSP
                            && (player.canCommandSenderUseCommand(2, "") || AetherConfig.sunAltarMultiplayer())) {
                            Aether.proxy.openSunAltar();
                        } else {
                            player.addChatComponentMessage(
                                new ChatComponentText(I18n.format("gui.sun_altar.permission")));
                        }
                    }
                    ...
                    if (world.isRemote) {
                        player
                            .addChatComponentMessage(new ChatComponentText(I18n.format("gui.sun_altar.cycle_catchup")));
                    }
                }
            }
        } else {
