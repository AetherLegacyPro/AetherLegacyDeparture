...
public class DivineEnchantmentTableContainer extends Container {
    ...
    public DivineEnchantmentTableContainer(InventoryPlayer p_i1811_1_, World p_i1811_2_, int p_i1811_3_, int p_i1811_4_,
        int p_i1811_5_) {
            ...
            if (itemstack != null && itemstack.isItemEnchantable()) {
                ...
                if (!this.worldPointer.isRemote) {
                    ...
                    for (j = -1; j <= 1; ++j) {
                        for (int k = -1; k <= 1; ++k) {
                            if ((j != 0 || k != 0)
                                && this.worldPointer.isAirBlock(this.posX + k, this.posY, this.posZ + j)
                                && this.worldPointer.isAirBlock(this.posX + k, this.posY + 1, this.posZ + j)) {
                    ...
                    for (j = 0; j < 6; ++j) // Bookshelf Bonus
                    {
                        this.enchantLevels[j] = EnchantmentHelper
                            .calcItemStackEnchantability(this.rand, j * 2, (int) power * 2, itemstack); //
                    }
    ...
    public boolean enchantItem(EntityPlayer p_75140_1_, int p_75140_2_) {
        ...
        if (this.enchantLevels[p_75140_2_] > 0 && itemstack != null
            && (p_75140_1_.experienceLevel >= 60 || p_75140_1_.capabilities.isCreativeMode)) // this.enchantLevels[p_75140_2_]
        {
            if (!this.worldPointer.isRemote) {
                List list = EnchantmentHelper
                    .buildEnchantmentList(this.rand, itemstack, this.enchantLevels[p_75140_2_]); // this.enchantLevels[p_75140_2_]
    ...
    public boolean canInteractWith(EntityPlayer p_75145_1_) {
        return this.worldPointer.getBlock(this.posX, this.posY, this.posZ) != BlocksAether.divine_enchantment_table
            ? false
            : p_75145_1_.getDistanceSq((double) this.posX + 0.5D, (double) this.posY + 0.5D, (double) this.posZ + 0.5D)
                <= 64.0D;
    }
