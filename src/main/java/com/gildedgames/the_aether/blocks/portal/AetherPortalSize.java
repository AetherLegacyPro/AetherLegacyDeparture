...
import com.gildedgames.the_aether.util.WorldUtils;
...
public class AetherPortalSize {
	...
	protected int getDistanceUntilEdge(int x, int y, int z, int leftDir) {
		...
		for (i1 = 0; i1 < 22; ++i1) {
			block = WorldUtils.getBlockSafe(this.world, x + j1 * i1, y, z + k1 * i1);

			if (!this.isEmptyBlock(block)) {
				break;
			}

			Block block1 = WorldUtils.getBlockSafe(this.world, x + j1 * i1, y - 1, z + k1 * i1);
	...
	protected int calculatePortalHeight() {
		...
		for (this.height = 0; this.height < 21; ++this.height) {
			for (int i = 0; i < this.width; ++i) {
				...
				Block block = WorldUtils.getBlockSafe(this.world, k, this.bottomLeft.posY + this.height, l);
				...
				if (i == 0) {
					block = WorldUtils.getBlockSafe(this.world, k + Direction.offsetX[BlockPortal.field_150001_a[this.axis][0]], this.bottomLeft.posY + this.height, l + Direction.offsetZ[BlockPortal.field_150001_a[this.axis][0]]);

					if (block != Blocks.glowstone) {
						break label24;
					}
				} else if (i == this.width - 1) {
					block = WorldUtils.getBlockSafe(this.world, k + Direction.offsetX[BlockPortal.field_150001_a[this.axis][1]], this.bottomLeft.posY + this.height, l + Direction.offsetZ[BlockPortal.field_150001_a[this.axis][1]]);
		...
		for (int j = 0; j < this.width; ++j) {
			...
			if (WorldUtils.getBlockSafe(this.world, i, k, l) != Blocks.glowstone) {
				this.height = 0;
				break;
			}
		}
	...
	public void placePortalBlocks() {
		for (int i = 0; i < this.width; ++i) {
			...
			for (int l = 0; l < this.height; ++l) {
				...
				WorldUtils.setBlockSafe(this.world, j, i1, k, BlocksAether.aether_portal, this.axis, 2);
			}
		}
	}
