...
public abstract class BlockMultiTileEntity extends BlockContainer {
    ...
    public void setBlockSize(final float minX, final float minY, final float minZ, final float maxX, final float maxY,
        final float maxZ) {
                ...
                for (int y2 = minY; y2 < maxY; ++y2) {
                    for (int z2 = minZ; z2 < maxZ; ++z2) {
                        if (world.getBlock(x2, y2, z2)
                            .onBlockActivated(world, x2, y2, z2, entityplayer, par6, par7, par8, par9)) {
                            return true;
                        }
                    }
                }
    ...
    public void breakBlock(final World world, final int x, final int y, final int z, final Block blockID,
        final int blockMetadata) {
        final TileEntityMultiBlock tileEntity = this.getLinkedTileEntity(world, x, y, z);
        if (tileEntity != null) {
            tileEntity.setSize(this.size);
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(final World world, final int x, final int y, final int z) {
        ...
        if (tileEntity instanceof TileEntityMultiBlock multiTileEntity) {
            ...
            return this.isDummyBlock(world, x, y, z) ? super.getCollisionBoundingBoxFromPool(world, x, y, z)
                : blockSize;
        }
        return super.getCollisionBoundingBoxFromPool(world, x, y, z);
    }
