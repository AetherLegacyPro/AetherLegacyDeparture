...
public class AetherEventHandler {
    ...
    @SidedProxy(
        clientSide = "com.gildedgames.the_aether.ClientProxy",
    ...
    public void onEntityInteract(EntityInteractEvent event) {
            ...
            if (itemstack != null && itemstack.getItem() == ItemsAether.aether_spawn_egg) {
                if (!event.entityPlayer.worldObj.isRemote) {
                    ...
                    if (oclass != null && oclass.isAssignableFrom(this.getClass())) {
                        EntityAgeable entityageable = ((EntityAgeable) event.target)
                            ...
                            if (!event.entityPlayer.capabilities.isCreativeMode) {
                                ...
                                if (itemstack.stackSize <= 0) {
                                    event.entityPlayer.inventory
                                        .setInventorySlotContents(event.entityPlayer.inventory.currentItem, null);
                                }
                            }
                        }
                    }
        ...
        if (event.target instanceof EntityCow || event.target instanceof EntityFlyingCow) {
            ...
            if (heldItem != null && heldItem.getItem() == ItemsAether.skyroot_bucket
                && EnumSkyrootBucketType.getType(heldItem.getItemDamage()) == EnumSkyrootBucketType.Empty) {
                ...
                if (heldItem.stackSize <= 0) {
                    player.inventory.setInventorySlotContents(
                        ...
                        new ItemStack(ItemsAether.skyroot_bucket, 1, EnumSkyrootBucketType.Milk.meta));
    ...
    public void onFillBucket(FillBucketEvent event) {
        ...
        boolean isWater = (!AetherConfig.activateOnlyWithSkyroot() && stack.getItem() == Items.water_bucket)
            || (stack.getItem() == ItemsAether.skyroot_bucket && stack.getItemDamage() == 1);
        boolean isLava = stack.getItem() == Items.lava_bucket;

        boolean validDimension = (player.dimension == AetherConfig.getTravelDimensionID()
    ...
    public void onEntityDropLoot(LivingDropsEvent event) {
        ...
        if (currentItem.getItem() instanceof ItemSkyrootSword && currentItem.getItem() instanceof ItemTippedSkyrootSword
            && !(entity instanceof EntityPlayer)
            && !(entity instanceof EntityWither)
            && !(entity instanceof EntityValkyrie)) {
            for (EntityItem items : event.drops) {
                ItemStack stack = items.getEntityItem();
                if (!(stack.getItem() instanceof ItemDungeonKey) && stack.getItem() != ItemsAether.victory_medal
        ...
        if (currentItem.getItem() instanceof ItemAmplifiedSkyrootSword && !(entity instanceof EntityPlayer)
            && !(entity instanceof EntityWither)
            && !(entity instanceof EntityValkyrie)) {
            for (EntityItem items : event.drops) {
                ItemStack stack = items.getEntityItem();
    ...
    public void onEntityStruckByLightning(EntityStruckByLightningEvent event) {
        if (event.entity instanceof EntityPlayer player) {
            ...
            if (currentStack != null) {
                ...
                if (currentItem == ItemsAether.lightning_knife || currentItem == ItemsAether.lightning_sword
                    || currentItem == ItemsAether.tipped_lightning_sword
                    || currentItem == ItemsAether.amplified_lightning_sword) {
                    event.setCanceled(true);
                    return;
                }
            }
            IAccessoryInventory accessoryInventory = PlayerAether.get(player)
            ...
            || event.entity instanceof EntityEliteValkyrie
            || event.entity instanceof EntityDivineValkyrieQueen
            || event.entity instanceof EntityElysianGuardian
            || event.entity instanceof EntityIrk
            || event.entity instanceof EntityGenesisDragon
    ...
    public void onEntityAttack(AttackEntityEvent event) {
        ...
        if (heldItem == ItemsAether.flaming_sword || heldItem == ItemsAether.tipped_flaming_sword
            || heldItem == ItemsAether.amplified_flaming_sword) {
            ...
            String s = EntityList.getEntityString(event.target)
                .toLowerCase();
            if (s != null && (s.contains("pig") || s.contains("phyg")
                || s.contains("taegore")
                || s.contains("oglin")
                || s.contains("piglin")
                || event.target.getUniqueID()
                    .toString()
                    .equals("1d680bb6-2a9a-4f25-bf2f-a1af74361d69"))) {
                if (event.target.worldObj.isRemote) {
                    for (int j = 0; j < 20; j++) {
                        ...
                        event.target.worldObj.spawnParticle(
                            "flame",
                            (event.target.posX + (double) (itemRand.nextFloat() * event.target.width * 2.0F))
                                - (double) event.target.width
                                - d * d3,
                            (event.target.posY + (double) (itemRand.nextFloat() * event.target.height)) - d1 * d3,
                            (event.target.posZ + (double) (itemRand.nextFloat() * event.target.width * 2.0F))
                                - (double) event.target.width
                                - d2 * d3,
                            d,
                            d1,
    ...
    private void performTimeSet(PlayerWakeUpEvent event, World world, WorldServer worldServer) {
        if (world.getGameRules()
            .getGameRuleBooleanValue("doDaylightCycle") && event.entityPlayer.isPlayerFullyAsleep()) {
